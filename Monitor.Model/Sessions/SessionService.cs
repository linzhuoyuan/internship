...
namespace Monitor.Model.Sessions
{
    public class SessionService : ISessionService, ISessionHandler
    {
        ...
        public void OpenHistories(HistoriesSessionParameters parameters)
        {
            if (_session != null)
            {
                // Another session is open.
            ...
            var session = new HistoriesSession(this, _resultConverter, parameters);
            OpenSession(session);
        }
        ...
        {
            get => _session?.State == SessionState.Subscribed;
            set
            {
