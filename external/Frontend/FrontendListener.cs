...
namespace Quantmom.Frontend
{
    public class FrontendListener : LongRunningTask
    {
        private class FrontendMonitor : IDisposable
        {
            ...
            private Task? _task;
            private readonly Dictionary<AsyncSocket, string> _sessions = new(100);
        ...
        private void PrintFrontendEvent(FrontendEvent e, bool sendMode)
        {
            ...
            if (data[0] == 1 || data[0] == 2 || data[0] == 14)
            {
                return;
            }
            ...
            sb.Append(size);
            sb.Append(",Data[0-7]:");
            sb.Append(BitConverter.ToString(data, 0, Math.Min(size, 8)));
        ...
        public FrontendListener(string address, ILogger logger, bool debugMode = false)
        {
            ...
            ReceiveQueue = new BufferBlock<FrontendEvent>();
            SendQueue = new BufferBlock<FrontendEvent?>();
        }
