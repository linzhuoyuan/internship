    ...
    public class EqualWeightingPortfolioConstructionModelTests
    {
        ...
        public void AutomaticallyRemoveInvestedWithNewInsights(Language language, InsightDirection direction)
        {
            ...
            var expectedTargets = _algorithm.Securities.Select(x => {
                ...
                var quantity = x.Key.Value == "SPY" ? 0 : (int)direction * Math.Floor(amount * (1 - _algorithm.Settings.FreePortfolioValuePercentage) / x.Value.Price);
                return new PortfolioTarget(x.Key, quantity);
            });

            // Do no include SPY in the insights
            insights = _algorithm.Securities.Keys
                .Where(x => x.Value != "SPY")
                .Select(x => GetInsight(x, direction, _algorithm.UtcTime))
                .ToArray();

            var actualTargets = _algorithm.PortfolioConstruction
                .CreateTargets(_algorithm, insights);
        ...
        public void AutomaticallyRemoveInvestedWithoutNewInsights(Language language)
        {
            ...
            var insights = new[]
            {
                GetInsight(Symbols.SPY, InsightDirection.Up, _algorithm.UtcTime)
            };
            ...
            var actualTargets = _algorithm.PortfolioConstruction
                .CreateTargets(_algorithm, new Insight[0]);
        ...
        public void LongTermInsightPreservesPosition(Language language)
        {
            ...
            var targets = _algorithm.PortfolioConstruction
                .CreateTargets(_algorithm, insights)
                .ToList();
            ...
            {
                PortfolioTarget.Percent(_algorithm, Symbols.SPY, -1m)
            };
