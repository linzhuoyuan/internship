...
namespace QuantConnect.Tests.Algorithm.Framework.Execution
{
    ...
    public class StandardDeviationExecutionModelTests
    {
            ...
            decimal expectedTotalQuantity)
        {
            ...
            orderProcessor
                .Setup(m =>
                    m.GetOpenOrders(It.IsAny<Func<Order, bool>>()))
            ...
            MarketDataType marketDataType)
        {
            ...
            historyProvider
                .Setup(m =>
                    m.GetHistory(
                        ...
                        It.IsAny<DateTimeZone>()))
                .Returns(historicalPrices.Select((x, i) =>
                    new Slice(time.AddMinutes(i), new List<BaseData> { Func(x, i) })));
            ...
            var changes = new SecurityChanges(
                new[] { security },
