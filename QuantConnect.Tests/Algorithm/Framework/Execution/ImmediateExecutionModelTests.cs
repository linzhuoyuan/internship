...
namespace QuantConnect.Tests.Algorithm.Framework.Execution
{
    ...
    public class ImmediateExecutionModelTests
    {
        ...
        {
            var actualOrdersSubmitted = new List<OrderRequest>();

            var orderProcessor = new Mock<IOrderProcessor>();
            orderProcessor.Setup(
                    m => m.Process(It.IsAny<OrderRequest>()))
                .Returns((OrderTicket)null)
                .Callback((OrderRequest request) => actualOrdersSubmitted.Add(request));
            ...
            decimal expectedTotalQuantity)
        {
            ...
            historyProvider
                .Setup(m =>
                    m.GetHistory(
                        It.IsAny<IEnumerable<HistoryRequest>>(),
                        It.IsAny<DateTimeZone>()))
            ...
            var orderProcessor = new Mock<IOrderProcessor>();
            orderProcessor
                .Setup(m => m.Process(It.IsAny<OrderRequest>()))
                .Returns((SubmitOrderRequest request) =>
                    new OrderTicket(algorithm.Transactions, request))
            ...
            orderProcessor
                .Setup(m => 
                    m.GetOpenOrderTickets(It.IsAny<Func<OrderTicket, bool>>()))
        ...
        public void PartiallyFilledOrdersAreTakenIntoAccount(Language language)
        {
            ...
            orderProcessor
                .Setup(m => 
                    m.Process(It.IsAny<SubmitOrderRequest>()))
                .Returns((SubmitOrderRequest request) =>
                    new OrderTicket(algorithm.Transactions, request))
                .Callback((OrderRequest request) => actualOrdersSubmitted.Add((SubmitOrderRequest)request));
            orderProcessor
