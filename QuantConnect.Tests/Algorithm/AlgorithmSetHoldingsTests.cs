    ...
    public class AlgorithmSetHoldingsTests
    {
        ...
        public class TestSecurityMarginModel : SecurityMarginModel
        {
            ...
            public new decimal GetMarginRemaining(
                SecurityPortfolioManager portfolio, Security security, OrderDirection direction)
            {
        ...
        public TestCaseData[] TestParameters
        {
            get {
                ...
                var ret = permutations
                    ...
                    .Select(row =>
                        new TestCaseData(row).SetName(string.Join("_", row)))
                    .ToArray();
            ...
            int leverage)
        {
            ...
            Mock<IOrderProcessor> orderProcessorMock = null;
            ...
            if (initialPosition != Position.Zero)
            {
                ...
                orderProcessorMock
                    .Setup(m => m.GetOrderById(It.IsAny<int>()))
                    .Returns(order);
                orderProcessorMock
...

            {
                ...
                orderDirection = finalPosition == Position.Long
                                 || finalPosition == Position.Zero
                                 && initialPosition == Position.Short
                    ? OrderDirection.Buy
                    : OrderDirection.Sell;
                orderQuantity = algorithm.CalculateOrderQuantity(_symbol, targetPercentage);
                order = new MarketOrder(_symbol, orderQuantity, DateTime.UtcNow);
