...
namespace QuantConnect.Algorithm
{
    public partial class QCAlgorithm
    {
        ...
        /// The data is added with a default time zone of Utc (Eastern Daylight Savings Time)
        /// </summary>
        ...
        {
            return AddData(type, symbol, resolution, TimeZones.Utc, false, 1m);
        }
        ...
        /// will be executed on day changes in the Utc time zone (<see cref="TimeZones.Utc"/>
        /// </summary>
        ...
        /// <param name="isSlice">return slice or tradeBar </param>
        /// <returns>A python dictionary with pandas DataFrame containing the requested historical data</returns>
        public PyObject History(PyObject tickers, int periods, Resolution? resolution = null, Boolean isSlice = true)
        {
            if (isSlice)
            {
            ...
            {
                var symbols = GetSymbolsFromPyObject(tickers).FirstOrDefault();
                return PandasConverter.GetDataFrame(History(symbols, periods, resolution));
            }
        }
        ...
        /// <param name="isSlice">return slice or tradeBar </param>
        /// <returns>A python dictionary with pandas DataFrame containing the requested historical data</returns>
        public PyObject History(PyObject tickers, TimeSpan span, Resolution? resolution = null, Boolean isSlice = true)
        {
            if (isSlice)
            {
                var symbols = GetSymbolsFromPyObject(tickers);
        ...
        /// <param name="resolution">The resolution to request</param>
         /// <param name="isSlice">return slice or tradeBar </param>
        /// <returns>A python dictionary with pandas DataFrame containing the requested historical data</returns>
        public PyObject History(PyObject tickers, DateTime start, DateTime end, Resolution? resolution = null, Boolean isSlice = true)
        {
            if (isSlice)
            {
                var symbols = GetSymbolsFromPyObject(tickers);
        ...
        public string Download(string address, PyObject headers, string userName, string password)
        {
            ...
            if (headers != null)
            {
                using (Py.GIL())
                {
                    ...
                    {
                        foreach (PyObject pyKey in headers.GetIterator())
                        {
        ...
        private Type CreateType(PyObject pyObject)
        {
            ...
            if (!_pythonActivators.TryGetValue(pyObject.Handle, out var pythonType))
            {
                AssemblyName an;
                using (Py.GIL())
                {
