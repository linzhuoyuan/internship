...
namespace QuantConnect.Algorithm
{
    ...
    public partial class QCAlgorithm : MarshalByRefObject, IAlgorithm
    {
        ...
        private readonly RunMode _runMode = RunMode.Series;
        private bool _locked;
        private bool _liveMode;
        private bool _simulationMode;
        private bool _autoOpenClose;
        private string _algorithmId = "";
        private ConcurrentQueue<string> _debugMessages = new();
        private ConcurrentQueue<string> _logMessages = new();
        private ConcurrentQueue<string> _errorMessages = new();
        ...
        private readonly string _dingServerAddress;
        ...
        public QCAlgorithm()
        {
            ...
            _timeKeeper = new TimeKeeper(_startDate, new[] { TimeZones.Utc });
            // set our local time zone
            _localTimeKeeper = _timeKeeper.GetLocalTimeKeeper(TimeZones.Utc);
            ...
            else if (PortfolioManagerName == "deribit")
            {
                Portfolio = new DeribitSecurityPortfolioManager(Securities, Transactions, DefaultOrderProperties);
            }
            else if (PortfolioManagerName == "mom")
            {
                Portfolio = new MomSecurityPortfolioManager(Securities, Transactions, DefaultOrderProperties);
            }

            TradeRecords = new ConcurrentDictionary<string, TradeRecord>();
            OrderRecords = new ConcurrentDictionary<string, OrderRecord>();
            ...
            SetSimulationMode();
            _dingServerAddress = Config.Get("momcrypto-historydata-server", "");
        }
        ...
        /// 
        /// </summary>
        public virtual string PortfolioManagerName
        {
            get;
        }
        ...
        /// brokerage trade
        /// </summary>
        public ConcurrentDictionary<string, TradeRecord> TradeRecords { get; }

        /// <summary>
        /// brokerage order
        /// </summary>
        public ConcurrentDictionary<string, OrderRecord> OrderRecords { get; }
        ...
        /// </summary>
        public DateRules DateRules => Schedule.DateRules;

        /// <summary>
        /// Gets the time rules helper object to make specifying times for events easier
        /// </summary>
        public TimeRules TimeRules => Schedule.TimeRules;
        ...
        public ICryptoChainProvider CryptoChainProvider { get; private set; }
        ...
        public DateTime Time => _localTimeKeeper.LocalTime;
        ...
        /// is <see cref="TimeZones.Utc"/>
        /// </summary>
        public DateTimeZone TimeZone => _localTimeKeeper.TimeZone;
        ...
        public bool AutoOpenClose => _autoOpenClose;
        ...
        {
            get => this._runtimeError;
            set
            {
                // Note: This translate a HResult int32 to an error code ( use truncated lower-16-bit status code (code & 0xFFFF)
                //       ref: https://docs.microsoft.com/en-us/windows/win32/com/structure-of-com-error-codes).
                this.ExitCode = System.Runtime.InteropServices.Marshal.GetHRForException(value) & ((1 << 16) - 1);
        ...
        public virtual void PostInitialize()
        {
            ...
            if (Benchmark == null)
            {
                if (_benchmarkSymbol != null)
                {
                    ...
                    // the desired market.
                    if (!Securities.TryGetValue(_benchmarkSymbol, out var security))
                    {
        ...
        public void SetCryptoChainProvider(ICryptoChainProvider cryptoChainProvider)
        {
            CryptoChainProvider = cryptoChainProvider;
        }
        ...
        public virtual void OnData(Dividends data)
        {
        ...
        public void SetStartDate(DateTime start)
        {
            ...
            if (_liveMode && !_simulationMode) return;
            ...
            if (!_locked)
            {
                // this is only for backtesting or simulation_mode
                if (!LiveMode || _simulationMode)
                {
                    _startDate = start;
                    SetDateTime(_startDate.ConvertToUtc(TimeZone));
                }
        ...
        {
            _simulationMode = Config.GetBool("simulation_mode");
            if (_simulationMode
                && Config.TryGetValue("simulation_start", out string simulationStart)
                && !string.IsNullOrEmpty(simulationStart))
            {
                _startDate = DateTime.ParseExact(simulationStart, "yyyy-MM-dd", null);
            ...
            DataNormalizationMode dataNormalizationMode = DataNormalizationMode.Adjusted)
        {
            ...
            try
            {
                ...
                if (properties != null)
                    symbol.SymbolProperties = properties;
        ...
        public Security AddSecurity(Symbol symbol, Resolution resolution = Resolution.Minute, bool fillDataForward = true, decimal leverage = Security.NullLeverage, bool extendedMarketHours = false)
        {
            ...
            if (isCanonical)
            {
                ...
                if (!UniverseManager.TryGetValue(symbol, out var universe) && _pendingUniverseAdditions.All(u => u.Configuration.Symbol != symbol))
                {
        ...
        public void AddSubscription(string ticker, Resolution resolution = Resolution.Minute, bool fillDataForward = true, decimal leverage = 0m, bool extendedMarketHours = false, DataNormalizationMode dataNormalizationMode = DataNormalizationMode.Adjusted)
        {
            var security = Securities[ticker];
            AddSubscription(security,
                resolution,
                fillDataForward,
                extendedMarketHours,
        ...
        public void AddSubscription(Symbol symbol, Resolution resolution = Resolution.Minute, bool fillDataForward = true, decimal leverage = 0m, bool extendedMarketHours = false, DataNormalizationMode dataNormalizationMode = DataNormalizationMode.Adjusted)
        {
        ...
        public void AddSubscription(Security security, Resolution resolution = Resolution.Minute, bool fillDataForward = true, bool extendedMarketHours = false, DataNormalizationMode dataNormalizationMode = DataNormalizationMode.Adjusted)
        {
            var configs = SubscriptionManager
                .SubscriptionDataConfigService
                .Add(security.Symbol, resolution, false, false, dataNormalizationMode: DataNormalizationMode.Raw);
            security.AddData(configs[0]);
        }
        ...
        public Equity AddEquity(string ticker, Resolution resolution = Resolution.Minute, string market = null, bool fillDataForward = true, decimal leverage = 0m, bool extendedMarketHours = false, DataNormalizationMode dataNormalizationMode = DataNormalizationMode.Adjusted)
        {
            return AddSecurity<Equity>(SecurityType.Equity, ticker, resolution, market, fillDataForward, leverage, extendedMarketHours, dataNormalizationMode);
        }
        ...
        /// <param name="fillDataForward">If true, returns the last available data even if none in that time-slice. Default is <value>true</value></param>
        /// <param name="leverage">The requested leverage for this equity. Default is set by <see cref="SecurityInitializer"/></param>
        ...
        public Option AddOption(string underlying, Resolution resolution = Resolution.Minute, string market = null, bool fillDataForward = true, decimal leverage = 0m)
        {
            ...
            // add this security to the user defined universe
            if (!UniverseManager.TryGetValue(canonicalSymbol, out var universe) && _pendingUniverseAdditions.All(u => u.Configuration.Symbol != canonicalSymbol))
            {
        ...
        public Future AddPerpetual(string ticker, Resolution resolution = Resolution.Minute, string market = null,
            bool fillDataForward = true, decimal leverage = 0m)
        {
            if (market == null)
            {
            ...
            {
                symbol = QuantConnect.Symbol.Create(ticker, SecurityType.Future, market, ticker, true);
            }
            ...
            var configs = SubscriptionManager.SubscriptionDataConfigService.Add(symbol,
                ...
                fillDataForward,
                isFilteredSubscription: false);
            var security = (Future)Securities.CreateSecurity(symbol, configs, leverage);
        ...
        public Option AddOptionContract(Symbol symbol, Resolution resolution = Resolution.Minute, bool fillDataForward = true, decimal leverage = 0m)
        {
            ...
            if (!Securities.TryGetValue(underlying, out var equity))
            {
                if (underlying.SecurityType == SecurityType.Equity)
                {
                    equity = AddEquity(underlying.Value, resolution, underlying.ID.Market, false);
                }
                else if (underlying.SecurityType == SecurityType.Crypto)
                {
                    equity = AddCrypto(underlying.Value, resolution, underlying.ID.Market, false);
                }
        ...
        public Crypto AddCrypto(string ticker, Resolution resolution = Resolution.Minute, string market = null, bool fillDataForward = true, decimal leverage = 0m, DataNormalizationMode dataNormalizationMode = DataNormalizationMode.Adjusted)
        {
            return AddSecurity<Crypto>(SecurityType.Crypto, ticker, resolution, market, fillDataForward, leverage, false, dataNormalizationMode: dataNormalizationMode);
        }
        ...
        public Security AddData<T>(string ticker, Resolution resolution, bool fillDataForward, decimal leverage = 1.0m)
            where T : IBaseData, new()
        {
            return AddData<T>(ticker, resolution, TimeZones.Utc, fillDataForward, leverage);
        }
        ...
        private T AddSecurity<T>(
            ...
            decimal leverage,
            bool extendedMarketHours,
            DataNormalizationMode dataNormalizationMode = DataNormalizationMode.Adjusted) where T : Security
        {
            if (market == null)
            {
                if (!BrokerageModel.DefaultMarkets.TryGetValue(securityType, out market))
            ...
            if (!SymbolCache.TryGetSymbol(ticker, out var symbol)
                || symbol.id.Market != market
                || symbol.id.SecurityType != securityType)
            {
                symbol = QuantConnect.Symbol.Create(ticker, securityType, market);
            }
            ...
            var configs = SubscriptionManager
                .SubscriptionDataConfigService
                .Add(symbol, resolution, fillDataForward, extendedMarketHours, dataNormalizationMode: dataNormalizationMode);
            var security = Securities.CreateSecurity(symbol, configs, leverage);
        ...
        {
            RunTimeError = exception ?? throw new ArgumentNullException("Algorithm.SetRunTimeError(): Algorithm.RunTimeError cannot be set to null.");
        }
        ...
        private void EmitInsightBasedOnFill(OrderEvent orderEvent)
        {
            if (_emitAutogeneratedOrderBasedInsight
                ...
                && orderEvent.Status is OrderStatus.Filled or OrderStatus.PartiallyFilled)
            {
        ...
        {
            Transactions.Query(AlgorithmQueryType.OpenOrder, requestId);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        public virtual void OnQueryOpenOrder(Order order, string requestId)
        {
        }
        ...
        {
            Transactions.Query(AlgorithmQueryType.Holding, requestId);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <param name="requestId"></param>
        public virtual void OnQueryAccountHolding(Holding holding, string requestId)
        {
        }
        ...
        {
            Transactions.Query(AlgorithmQueryType.CashBalance, requestId);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <param name="requestId"></param>
        public virtual void OnQueryCashBalance(CashAmount balance, string requestId)
        {
        }

        public virtual bool TradingIsReady()
        {
            return Transactions.TradingIsReady();
        }

        public RequestResult<QuoteRequest> SendQuoteRequest(QuoteRequest request)
        {
            return Transactions.OptionQuoteRequest(request);
        }

        public RequestResult<Quote> AcceptRequestQuote(string quoteId)
        {
            return Transactions.AcceptRequestQuote(quoteId);
        }

        public RequestResult<QuoteRequest> CancelQuoteRequest(string requestId)
        {
            return Transactions.CancelQuoteRequest(requestId);
        }

        public RequestResult<List<Quote>> QueryOptionQuote(string requestId)
        {
            return Transactions.QueryOptionQuote(requestId);
        }

        public RequestResult<List<OptionPosition>> QueryOptionPosition()
        {
            return Transactions.QueryOptionPosition();
        }

        public void ChangeLeverage(string symbol, int leverage, string exchange = "")
        {
            Transactions.ChangeLeverage(symbol, leverage, exchange);
        }

        public void Transfer(decimal amount, UserTransferType type = UserTransferType.Spot2UmFuture, string currency = "USDT")
        {
            Transactions.Transfer(amount, type, currency);
        }

        public virtual void OnTransferCompleted(decimal amount, UserTransferType type, string currency)
        {
        }

        public virtual void OnTransferFailed(decimal amount, UserTransferType type, string currency, string error)
        {
        }

        public virtual void OnRiskDegreeChanged(decimal riskDegree, decimal userRiskDegree)
        {
        ...
        {
            Transactions.ExchangeQuery(AlgorithmQueryType.ExchangeOpenOrder, uId, requestId);
        }

        public virtual void OnQueryExchangeOpenOrders(Order order, string requestId, bool isLast)
        {
        ...
        {
            Transactions.ExchangeQuery(AlgorithmQueryType.ExchangeHolding, uId, requestId);
        }

        public virtual void OnQueryExchangeAccountHolding(Holding holding, string requestId, bool isLast)
        {
        ...
        {
            Transactions.ExchangeQuery(AlgorithmQueryType.ExchangeCashBalance, uId, requestId);
        }

        public virtual void OnQueryExchangeCashBalance(CashAmount balance, string requestId, bool isLast)
        {

        }

        public virtual void OnSymbolListed(List<Symbol> symbols)
        {
        }

        public virtual void OnSymbolDelisted(List<Symbol> symbols)
        {
        }

        public virtual void OnSettlementNotice(
        ...
        /// 
        /// </summary>
        public virtual void OnProgramExit()
        {
        }
        ...
        public Equity AblAddEquity(string ticker, Resolution resolution = Resolution.Minute, string market = null, bool fillDataForward = true, decimal leverage = 0m, bool extendedMarketHours = false, DataNormalizationMode dataNormalizationMode = DataNormalizationMode.Adjusted)
        {
            var equity = AddEquity(
                ...
                extendedMarketHours,
                dataNormalizationMode);
            BrokerageSubscribe?.Invoke(equity.symbol);
            return equity;
        }

        public void AblUnsubscribe(Symbol symbol)
        {
            BrokerageUnsubscribe?.Invoke(symbol);
        }

        public Action<Symbol> BrokerageSubscribe { get; set; }
        public Action<Symbol> BrokerageUnsubscribe { get; set; }

        public void LoadBacktestInitPosition()
        {
            if (!_liveMode)
            {
                BacktestInitPositionAction?.Invoke();
            }
        }

        public Action BacktestInitPositionAction { get; set; }
        public CancellationTokenSource ExitTokenSource { get; set; }

        public DateTimeOffset LiveStartTime { get; set; } = DateTimeOffset.MinValue;

        public DateTimeOffset LiveStopTime { get; set; } = DateTimeOffset.MaxValue;
        
        public static TimeSpan ParseTimeSpan(string timeString)
        {
            var parts = timeString.Split('.');
            var formatString = parts.Length > 1 ? @"d\.hh\:mm\:ss" : @"hh\:mm\:ss";
            return TimeSpan.ParseExact(timeString, formatString, CultureInfo.InvariantCulture);
        }
        
        private static DateTimeOffset ParseTimeWithZone(string value)
        {
            var items = value.Split(',', ' ');
            if (items.Length > 1)
            {
                var zone = TimeZones.GetDateTimeZone(items[1]);
                var time = ParseTimeSpan(items[0]);
                var utcTime = TimeZones.GetZoneToday(zone).Add(time).UtcDateTime;
                return TimeZones.GetDateTimeOffset(zone, utcTime);
            }
            else
            {
                var time = TimeSpan.ParseExact(value, "d.hh:mm:ss", null);
                return new DateTimeOffset(DateTime.UtcNow.Date.Add(time), TimeSpan.Zero);
            }
        }

        private void SetLiveTimeRange()
        {
            if (Config.TryGetValue("live-start-time", out string startTime) && !string.IsNullOrEmpty(startTime))
            {
                LiveStartTime = ParseTimeWithZone(startTime);
            }
            if (Config.TryGetValue("live-stop-time", out string stopTime) && !string.IsNullOrEmpty(stopTime))
            {
                LiveStopTime = ParseTimeWithZone(stopTime);
            }
        }
        ...
        public void WaitForLiveStart(Isolator isolator, DateTimeOffset? utcNow = null)
        {
            ...
            var liveStartUtc = LiveStartTime.UtcDateTime;
            if (now < liveStartUtc)
            {
                var timespan = liveStartUtc - now;
                Debug($"Algorithm will start at : {LiveStartTime}, waiting: {timespan}.");
                Thread.Sleep(timespan);
            }

            var liveStopUtc = LiveStopTime.UtcDateTime;
            if (liveStopUtc == DateTimeOffset.MaxValue)
            {
                return;
            }

            Debug($"Algorithm will exit at {LiveStopTime}.");
            if (now < liveStopUtc)
            {
                var timespan = liveStopUtc - now;
                Task.Delay(timespan).ContinueWith(_ => isolator.CancellationTokenSource.Cancel());
            }
            else
            {
                isolator.CancellationTokenSource.Cancel();
            }
        }


        public IEnumerable<BrokerageAccountInfo> BrokerageAccountInfos { get; set; }

        public IDictionary<string, BrokerageAccountInfo> BrokerageAccountMap { get; set; } =
            new Dictionary<string, BrokerageAccountInfo>();

        public BrokerageAccountInfo GetBrokerageAccount(Security security, bool fundAccount = false)
        {
            return GetBrokerageAccount(security.Symbol, fundAccount);
        }

        public BrokerageAccountInfo GetBrokerageSpotAccount(string currency, bool fundAccount = false)
        {
            return GetBrokerageAccount("spot", currency, fundAccount);
        }

        public BrokerageAccountInfo GetBrokerageFuturesAccount(string currency, bool fundAccount = false)
        {
            return GetBrokerageAccount("futures", currency, fundAccount);
        }

        public BrokerageAccountInfo GetBrokerageAccount(string accountType, string currency, bool fundAccount = false)
        {
            var key = $"{accountType}-{currency}".ToLower();
            if (fundAccount)
            {
                key = $"fund-{key}";
            }
            return BrokerageAccountMap.TryGetValue(key, out var info) ? info : null;
        }

        public BrokerageAccountInfo GetBrokerageAccount(Symbol symbol, bool fundAccount = false)
        {
            ...
            return GetBrokerageAccount(accountType, symbol.SymbolProperties.QuoteCurrency, fundAccount);
        }

        public Func<Symbol, decimal> GetMarketPriceFromSnapshot { get; set; } = _ => 0m;
        ...
        public void SendDingMsg(string text, string token = null, string key = null)
        {
            try
            {
                ...
                new HttpClient
                {
                    BaseAddress = new Uri(_dingServerAddress, UriKind.Absolute)
                }.GetAsync(url).Wait();
            }
            catch (Exception e)
            {
                Error("SendDingMsg : " + e.Message);
            }
        }
    }
}
