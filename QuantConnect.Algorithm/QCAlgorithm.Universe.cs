...
namespace QuantConnect.Algorithm
{
    public partial class QCAlgorithm
    {
        ...
        private readonly object _pendingUniverseAdditionsLock = new();
        private readonly List<UserDefinedUniverseAddition> _pendingUserDefinedUniverseSecurityAdditions = new();
        private readonly List<Universe> _pendingUniverseAdditions = new();
        ...
        public void OnEndOfTimeStep()
        {
            ...
            lock (_pendingUniverseAdditionsLock)
            {
                ...
                foreach (var security in Securities
                             .Select(n => n.Value)
                             .Union(_pendingUserDefinedUniverseSecurityAdditions.Select(n => n.Security)))
                {
                    // check for any derivative securities and mark the underlying as raw
                    if (Securities.Any(n => n.Key.HasUnderlyingSymbol(security.Symbol)))
                    {
                    ...
                    if (security.Symbol.HasUnderlying)
                    {
                        ...
                        if (!Securities.TryGetValue(underlyingSymbol, out var underlyingSecurity))
                        {
                ...
                if (!UniverseManager.TryGetValue(universeSymbol, out universe))
                {
                    ...
                    if (universe == null)
                    {
                        ...
                        if (security.Type == SecurityType.Base)
                        {
                            ...
                            var symbolString = MarketHoursDatabase.GetDatabaseSymbolKey(subscriptionDataConfig.Symbol);
                            MarketHoursDatabase.SetEntry(
                                subscriptionDataConfig.Market, 
                                symbolString, 
                                subscriptionDataConfig.SecurityType, 
                                security.Exchange.Hours, 
                                subscriptionDataConfig.DataTimeZone);
                        }

                        universe = new UserDefinedUniverse(subscriptionDataConfig,
