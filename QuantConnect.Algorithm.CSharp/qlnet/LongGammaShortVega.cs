...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class LongGammaShortVega : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        private decimal order_unit = 100;
        private decimal lastHoldKUpper = 0m;
        private decimal lastHoldKLower = 0m;
        private DateTime lastHoldFrontMonth = new DateTime(2015, 1, 1);
        private int holdDaysLeft = 0;
        private decimal _deltaThres = 10m;
        private int _multi = 10000;
        private decimal stopWinThres = 0.05m;

        public override void Initialize()
        {
            SetTimeZone(TimeZones.Shanghai);
            SetCash(1000000);
            SetStartDate(2015, 02, 10);
            SetEndDate(2021, 02, 05);
        ...
        public override void OnData(Slice slice)
        {
            ...
            {
                if (slice.Time.Minute == 31 || slice.Time.Minute == 59)
                {
                    ...
                    if (profitPct > stopWinThres)
                    {
                        foreach (var h in holdings)
                        {
                            ...
                            MarketOrder(con.Symbol, -h.Holdings.Quantity, tag: "stop profit: close all strangle position");
                        }
                        holdDaysLeft = 0;
                    }
                }
                ...
                if (slice.Time.TimeOfDay == openTime)
                {
                    ...
                    foreach (var h in holdings)
                    {
                        // if expiry is really close, we close all position
                        var con = (
                        from optionContract in chain
                        where optionContract.Symbol == h.Symbol
                        select optionContract
                    ...
                    if (call_front != null && put_front != null && call_far != null && put_far != null)
                    {
                        ...
                        if (call_front_impvol != 0m && put_front_impvol != 0m && call_far_impvol != 0m && put_far_impvol != 0m)
                        {
                            ...
                            if (0.5m*(call_far_impvol + put_far_impvol) < 0.15m)
                            {
                                vega_hedge_ratio = 0.5m;
                            }
                    ...
                    if (trade_direction != 0)
                    {
                        ...
                        if (trade_direction == 1)
                        {
                            ...
                            {
                                var far_order_unit = decimal.ToInt32(order_unit * vega_hedge_ratio);
                                MarketOrder(call_front.Symbol, order_unit, tag: "buy front strangle");
                                MarketOrder(put_front.Symbol, order_unit, tag: "buy front strangle");
                                MarketOrder(call_far.Symbol, -far_order_unit, tag: "sell far strangle");
                                MarketOrder(put_far.Symbol, -far_order_unit, tag: "sell far strangle");
                                lastHoldKUpper = k_upper;
                                lastHoldKLower = k_lower;
                                lastHoldFrontMonth = expiry_date_front;
                                holdDaysLeft = 5;
                            }
                            else
                            {
