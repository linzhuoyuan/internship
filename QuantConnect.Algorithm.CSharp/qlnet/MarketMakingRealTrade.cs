...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class MarketMakingRealTrade : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        TimeSpan openTime = new DateTime(1, 1, 1, 14, 50, 0).TimeOfDay;
        private bool today_opened = false;
        ...
        public override void OnData(Slice slice)
        {
            ...
            {
                if (h.BidPrice == 0 || h.AskPrice == 0 || h.Price == 0)
                {
                    return;
                }
                unrealizedProfit += h.LongHoldings.UnrealizedProfit;
                cost += h.LongHoldings.HoldingsCost;
                unrealizedProfit += h.ShortHoldings.UnrealizedProfit;
                cost += h.ShortHoldings.HoldingsCost;
            }
            ...
            {
                var expiryList1 = chain.OrderBy(x => x.Expiry).Select(x => x.Expiry).Distinct().ToList();
                if (expiryList1.Count < 4)
                {
                    return;
                }
                var expiry_date_front1 = expiryList1[0];
                var tmp = SelectATMContracts(chain, expiry_date_front1, 0.05m);
                ...
                if (slice.Time.TimeOfDay >= openTime && today_opened == false)
                {
                    if (trade_direction != 0)
                    {
                        ...
                        if (trade_direction == -1)
                        {
                            ...
                            {
                                OpenSellLimit(call_front.Symbol, order_unit, call_front.BidPrice, tag: "sell straddle");
                                OpenSellLimit(put_front.Symbol, order_unit, put_front.BidPrice, tag: "sell straddle");
