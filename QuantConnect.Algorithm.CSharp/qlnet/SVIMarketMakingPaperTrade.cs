...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class SVIMarketMakingPaperTrade : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (slice.OptionChains.TryGetValue(OptionSymbol, out chain) && chain.Underlying.Price != 0)
            {
                ...
                for (int i = 0; i < strikes.Count; ++i)
                {
                    ...
                    var open_orders = Transactions.GetOpenOrders(callContract.Symbol);
                    if (open_orders.Count > 0)
                    {
                        var open_bids = open_orders.Where(x => x.Quantity > 0);
                        var open_asks = open_orders.Where(x => x.Quantity < 0);
                        // quote bid;
                        if (open_bids.Count() > 0)
                        {
                            // find bid order first
                            var open_order = open_bids.FirstOrDefault();
                            if (open_order.Price != theo_bid)
                            {
                                var order_ticket = Transactions.GetOrderTicket(open_order.Id);
                                order_ticket.Update(new UpdateOrderFields {
                                    LimitPrice = theo_bid,
                                    Tag = $"Change price for: {callContract.Symbol.Value} to new bid: {theo_bid}"
                                });
                            }
                        }
                        else
                        ...
                        // quote ask;
                        if (open_asks.Count() > 0)
                        {
                            // find bid order first
                            var open_order = open_asks.FirstOrDefault();
                            if (open_order.Price != theo_ask)
                            {
                                var order_ticket = Transactions.GetOrderTicket(open_order.Id);
                                order_ticket.Update(new UpdateOrderFields {
                                    LimitPrice = theo_ask,
                                    Tag = $"Change price for: {callContract.Symbol.Value} to new bid: {theo_ask}"
                                });
                            }
                        }
                        else
