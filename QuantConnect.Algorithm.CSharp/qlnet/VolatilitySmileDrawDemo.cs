...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class VolatilitySmileDrawDemo : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (slice.OptionChains.TryGetValue(OptionSymbol, out chain) && chain.Underlying.Price != 0)
            {
                ...
                List<double> observedVols = new InitializedList<double>(strikeList.Count, 0.0);
                List<double> localVols = new InitializedList<double>(strikeList.Count, 0.0);
                ...
                for (int i = 0; i < strikeList.Count; ++i)
                {
                    ...
                    observedVols[i] = decimal.ToDouble(callContract.ImpliedVolatility);
                    strikes[i] = decimal.ToDouble(strikeList[i]);
                }

                SviInterpolation svi = new SviInterpolation(strikes, strikes.Count, observedVols, _tau,
                ...
                {
                    localVols[i] = svi.value(strikes[i]);
                    Log($"strike: {strikes[i]} impliedVolatility: {observedVols[i]} localVol: {localVols[i]}");
                }

                CustomerChartData customerChartData = new CustomerChartData(slice.Time, "Strikes", "ImpliedVolatility", "LocalVolatility");
                customerChartData.XList = strikes.ToArray();
                customerChartData.YList = observedVols.ToArray();
                customerChartData.Y1List = localVols.ToArray();
