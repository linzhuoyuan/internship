...
{
    public class DeltaCollarStrategyLeverage
    {
        //是否选出期权
        ...
        public DeltaCollarStrategyLeverage(Underlying underlying)
        {
        ...
        public IList<string> DetectionStrategy(DateTime sliceTime, decimal underlyingPrice, double sigma, decimal markPrice,
            decimal balanceNow = -1, bool stopProfit = false, bool moveStrke = false, decimal updateStrikeRatio = 1m,
            IEnumerable<MarketHoursSegment> marketHourSegments = null, IList<DateTime> holidays = null, Dictionary<DateTime, TimeSpan> earlyCloses = null)
        {
            ...
            ExtremePrice = Math.Max(ExtremePrice, underlyingPrice);
            ...
            if (!FirstCrossLimitPrice && markPrice > LimitPrice)
            {
            ...
            {
                OldStrike = Math.Max(OldStrike, markPrice * updateStrikeRatio);
            }
            ...
            if (sliceTime >= ExpiryDateNow)
            {
                ...
                if (FirstCrossLimitPrice && (markPrice < LimitPrice || markPrice < OldStrike))
                {
                    // while (underlyingPrice * 1.1m > OldStrike && updateStrikeRatio < 1)
                    // {
                    //     OldStrike = OldStrike * 1.1m;
                    // }
                    // //OldStrike = Math.Max(OldStrike, LimitPrice);
                    // //Options[0].Strike = OldStrike;
                    ...
                    //     T2M = InitialT2M;
                    // }
                    LimitPrice = ExtremePrice * (Index * 0.01m + 1);
                    OldStrike = Math.Max(OldStrike, LimitPrice);
                }


                if (FirstCrossLimitPrice && markPrice > OldStrike)
                {
                    OldStrike = markPrice;
                }
            }
            ...
            if (!HasOption)
            {
                ...
                OldStrike = Math.Max(OldStrike, price);
                Options.Add(
            ...
            if (HasOption)
            {
                ...
                if (t < 0)
                {
                    throw new Exception($"Expired synthetic option!!!!!! ExpiryDateNow {ExpiryDateNow}, sliceTime {sliceTime}, t {t}, index {Index}, underlying {Underlying.CoinPair}");
                }
                ...
                else if (t > 0)
                {
                    ...
                    else if (TargetDelta < 0.1m)
                    {
                        if (TargetDelta > 0 || !moveStrke)
                        {
                                ...
                                OldStrike < ExtremePrice)
                            {
                                ...
                                CashDelta = (decimal) BSModel.GetDelta(S, (double) op.Strike, 0, r, sigma, t,
                                    BSModel.EPutCall.Put) * op.Volume + TargetUnderlyingVolume * op.Volume;
                            }*/
                        }
                        else
                        {
