...
{
    public class DeltaCollarStrategyBear
    {
        //是否选出期权
        ...
        public DeltaCollarStrategyBear(Underlying underlying)
        {
            ...
            IEnumerable<MarketHoursSegment> marketHourSegments = null, IList<DateTime> holidays = null, Dictionary<DateTime, TimeSpan> earlyCloses = null)
        {
            ...
            if (sliceTime >= ExpiryDateNow)
            {
                ...
                if ((markPrice > LimitPrice || markPrice > OldStrike) && FirstCrossLimitPrice)
                {
