...
{
    public class BinanceHybrid2 : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        protected Dictionary<string, List<DeltaCollarStrategyLeverage2>> BullStrategies = new();

        protected Dictionary<string, List<DeltaCollarStrategyBear2>> BearStrategies = new();
        ...
        public override void Initialize()
        {
            if (File.Exists(BullJsonPath))
            {
                ...
                BullStrategies = JsonConvert.DeserializeObject<List<DeltaCollarStrategyLeverage2>>(jsonList)
            ...
            if (File.Exists(BearJsonPath))
            {
                ...
                BearStrategies = JsonConvert.DeserializeObject<List<DeltaCollarStrategyBear2>>(jsonList)
                    .GroupBy(s => s.Underlying.CoinPair)
                    .ToDictionary(g => g.Key, g => g.ToList());
            }
                ...
                if (CoinPairs.Contains(coinPair))
                {
                    ...
                    if (_isFirstRuns[coinPair])
                    {
                            ...
                            if (!_volEstimators.ContainsKey(coinPair))
                            {
                                _volEstimators.Add(coinPair,
                                    new EWMAVolatilityEstimator((double)price,
                                        0.49 / _volMultiplier, Lambda));
                            }

                            BullStrategies[coinPair] = new List<DeltaCollarStrategyLeverage2>();
                            var balanceNow = BullAssetAllocationConfigMap[coinPair].Asset / price /
                                                10;
                            for (var i = 1; i <= 10; i++)
                            {
                                var strategy = new DeltaCollarStrategyLeverage2(_underlyings[coinPair]);
                        ...
                        if (_bearResetCoins.Contains(coinPair))
                        {
                            ...
                            BearStrategies[coinPair] = new List<DeltaCollarStrategyBear2>();
                            var balanceNow = BearAssetAllocationConfigMap[coinPair].Asset / price /
                                                10;
                            for (var i = 1; i <= 10; i++)
                            {
                                var strategy = new DeltaCollarStrategyBear2(_underlyings[coinPair]);
