...
[BrokerageFactory(typeof(UniswapBrokerageFactory))]
public class UniswapBrokerage : Brokerage, IDataQueueHandler
{
    ...
    private MySqlConnection local_db_main_net = new MySqlConnection("server=192.168.10.243;user=liangchen;database=twitter;port=3306;password=Wlc12588888");
    private MySqlConnection local_db_goerli = new MySqlConnection("server=192.168.10.243;user=liangchen;database=uniswap_goerli;port=3306;password=Wlc12588888");
    private MySqlConnection local_db_arbitrum = new MySqlConnection("server=192.168.10.243;user=liangchen;database=uniswap_arbitrum;port=3306;password=Wlc12588888");
    private MySqlConnection history_db_main_net = new MySqlConnection("server=192.168.10.243;user=liangchen;database=twitter;port=3306;password=Wlc12588888");
    private MySqlConnection history_db_goerli = new MySqlConnection("server=192.168.10.243;user=liangchen;database=uniswap_goerli;port=3306;password=Wlc12588888");
    private MySqlConnection history_db_arbitrum = new MySqlConnection("server=192.168.10.243;user=liangchen;database=uniswap_arbitrum;port=3306;password=Wlc12588888");
    ...
    private string RegularMessageToken = "info";

    [Parameter("asset-allocation-config-file")]
    public string AssetAllocationConfigFile = string.Empty;

    public UniswapBrokerage(IAlgorithm algorithm)
        : base(nameof(UniswapBrokerage)) {
        ...
        _algorithm.RequestSplitNum = (decimal tradeVolume, string coin_pair, decimal estimate_usd, decimal eth_price) => {
            ...
            try {
                // var response = clientHttp.DownloadString($"http://192.168.10.243:{http_port}/?wallet_addr={wallet_addr}&request_type=query_split_num&quote_symbol={quote_symbol}&quote_token_address={quote_token_address}&quote_token_name={quote_token_name}&quote_token_decimal={quote_token_decimal}&quote_amount={quote_amount}&max_splits={max_splits_str}&base_currency={base_currency}");
                
                var request = new HttpRequestMessage(HttpMethod.Get, $"http://192.168.10.243:{http_port}/?wallet_addr={wallet_addr}&request_type=query_split_num&quote_symbol={quote_symbol}&quote_token_address={quote_token_address}&quote_token_name={quote_token_name}&quote_token_decimal={quote_token_decimal}&quote_amount={quote_amount}&max_splits={max_splits_str}&base_currency={base_currency}&estimate_usd={estimate_usd_str}");
        ...
        switch (chain_id) {
            ...
            case 42161:
                ...
                break;
            default:
                throw new ArgumentException($"chain id error");
        }
        ...
        for(int i = 1; i < asset_allocations.Count(); i++) {
            ...
            if (!SymbolCache.TryGetSymbol(ticker, out ass_symbol)) {
                var new_add_symbol = Symbol.Create(ticker, SecurityType.Crypto, Market.Uniswap, ticker);
    ...
    private IList<BarData> GetHistoryBar(Symbol symbol, DateTime startUtc, DateTime endUtc, Resolution resolution) {   
        ...
        if ((endUtc - startUtc).TotalSeconds <= 10) {
            string conversion_rate_sql = $"select ts, price, amount from uniswap_price_oracle where concat(quote_token_identifier, base_token_identifier)=\'{symbol.value}\' order by ts desc limit 1;";
    ...
    public override List<CashAmount> GetCashBalance() {
        ...
        //     list.Add(cash);
        //     Console.WriteLine($"coin {ticker} balance:" + cash.Amount);
        // }
    ...
    public override bool PlaceOrder(Order order) {
        ...
        UniswapOrder uniswapOrder = new UniswapOrder{
        ...
        var json_order = Newtonsoft.Json.JsonConvert.SerializeObject(uniswapOrder);
        Console.WriteLine("before json order:" + uniswapOrder);
    ...
    private void AddSymbol() {
        ...
        var eth_symbol = Symbol.Create("ETHUSDC", SecurityType.Crypto, Market.Uniswap, "ETHUSDC");
        ...
        foreach (var ticker in tickers) {
            var symbol = Symbol.Create(ticker + base_currency, SecurityType.Crypto, Market.Uniswap, ticker + base_currency);
        ...
        // var _symbol = Symbol.Create("UNIUSDC", SecurityType.Crypto, Market.Uniswap, "UNIUSDC");
        // _symbol.SymbolProperties = new SymbolProperties("UNI", "USDC", 1, 0.000001m, 0.000001m);
        // SymbolCache.Set(_symbol.Value, _symbol);
        // SymbolPropertiesDatabase.FromDataFolder().Add(_symbol);
    }
        ...
        switch (chain_id) {
            case 1:
                if(this.local_db_main_net.Ping() == false) {
                    Console.WriteLine("sql health check main net sql connection closed, reconnecting");
            ...
            case 42161:
                if(this.local_db_arbitrum.Ping() == false) {
                    Console.WriteLine("sql health check arbitrum sql connection closed, reconnecting");
    ...
    public IEnumerable<BaseData> GetNextTicks() {
        ...
        while (_isConnected) {
            ...
            if(this.global_last_timestamp == int.MaxValue) {
                // 首次启动时，只取最新的一个价格
                sql = $"select a.quote_token_identifier,a.base_token_identifier,a.ts,a.price,a.amount, a.block_number FROM(SELECT*,concat(quote_token_identifier,base_token_identifier)AS pair_name FROM uniswap_price_oracle)a INNER JOIN(SELECT quote_token_identifier,base_token_identifier,max(ts)AS mts FROM uniswap_price_oracle GROUP BY base_token_identifier,quote_token_identifier)b ON a.base_token_identifier=b.base_token_identifier AND a.quote_token_identifier=b.quote_token_identifier AND a.ts=b.mts WHERE pair_name in(\"{subscribe_pairs_str}\");";
                // Console.WriteLine("query sql:" + sql);
            } else {
                // 已经有过datafeed进来时，按照上次读取的时间戳，取上次时间戳之后的价格
                sql = $"select quote_token_identifier, base_token_identifier, ts, price, amount, block_number from uniswap_price_oracle where ts >= {this.global_last_timestamp} and concat(quote_token_identifier, base_token_identifier) in(\"{subscribe_pairs_str}\") order by ts;";
                // Console.WriteLine("query sql:" + sql);
            }
    ...
    public static bool SupportSymbol(Symbol symbol) {
        ...
        return market is Market.Uniswap;
    }
    ...
    public void Subscribe(LiveNodePacket job, IEnumerable<Symbol> symbols) {
        ...
        foreach (var symbol in symbols) {
            ...
            if (!SupportSymbol(symbol)) {
                var new_add_symbol = Symbol.Create(symbol.value, SecurityType.Crypto, Market.Uniswap, symbol.value);
                new_add_symbol.SymbolProperties = new SymbolProperties(
