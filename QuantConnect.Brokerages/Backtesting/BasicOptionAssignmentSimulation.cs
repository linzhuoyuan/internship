...
namespace QuantConnect.Brokerages.Backtesting
{
    ...
    public class BasicOptionAssignmentSimulation : IBacktestingMarketSimulation
    {
        ...
        // we rescan portfolio for new contracts and expiration every month
        ...
        private static readonly Random Rand = new Random((int)12345);
        ...
        public bool IsReadyToSimulate(IAlgorithm algorithm)
        {
            ...
            {
                var expiration = algorithm.Securities.Select(x => x.Key)
                ...
                foreach (var expirationDate in expiration)
                {
                ...
                var randomizedScans = scans
                                    ...
                                    .OrderBy(x => x)
                                    .Select(x => x.AddMinutes(Rand.NextDouble() * _assignmentScanPeriod.TotalMinutes));
        ...
        public void SimulateMarketConditions(IBrokerage brokerage, IAlgorithm algorithm)
        {
            ...
            bool DeepItm(Symbol symbol)
            {
                var underlyingPrice = algorithm.Securities[symbol.Underlying].Close;

                var result = symbol.ID.OptionRight == OptionRight.Call ? (underlyingPrice - symbol.ID.StrikePrice) / underlyingPrice > _deepITM : (symbol.ID.StrikePrice - underlyingPrice) / underlyingPrice > _deepITM;
            ...
            algorithm.Securities
                ...
                .Where(x => x.Key.ID.SecurityType == SecurityType.Option &&
                            (x.Value as Option).Expiry - algorithm.UtcTime <= _priorExpiration)
            ...
            ICurrencyConverter currencyConverter)
        {
            ...
            var underlyingMarketOrder2 = new MarketOrder(underlying.Symbol, -underlyingQuantity, underlying.LocalTime.ConvertToUtc(underlying.Exchange.TimeZone));
