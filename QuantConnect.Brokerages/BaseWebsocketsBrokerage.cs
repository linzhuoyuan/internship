...
namespace QuantConnect.Brokerages
{
    ...
    public abstract class BaseWebsocketsBrokerage : Brokerage
    {
        ...
        /// </summary>
        public ConcurrentDictionary<long, Orders.Order> CachedOrderIDs = new ConcurrentDictionary<long, Orders.Order>();
        /// <summary>
        ...
        protected volatile bool _connectionLost;
        ...
        public BaseWebsocketsBrokerage(string wssUrl, IWebSocket websocket, RestClient restClient, string apiKey, string apiSecret, string market, string name) : base(name)
        {
            WebSocket = websocket;
                ...
                try
                {
                    while (!_cancellationTokenSource.IsCancellationRequested)
                    {
                        ...
                        if (!_connectionLost && elapsed > TimeSpan.FromSeconds(_heartbeatTimeout))
                        {
                            ...
                            {
                                _connectionLost = true; //断开连接标志和下次重连时间 ，下次循环走else if
                                ...
                                else
                                {
                                    if (DateTime.UtcNow > nextReconnectionAttemptUtcTime)
                                    {
                                        ...
                                        catch (Exception err)
                                        {
                                            ...
                                            nextReconnectionAttemptUtcTime = DateTime.UtcNow.AddSeconds(nextReconnectionAttemptSeconds*2);
                                            Log.Error(err);
                                        }
                                    }
                                }
