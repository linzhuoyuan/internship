...
namespace QuantConnect.Lean.Engine.Setup
{
    ...
    public class BrokerageSetupHandler : ISetupHandler
    {
        ...
        private static readonly string _dataFeedsConfigString = Config.Get("security-data-feeds");
        ...
        public virtual IAlgorithm CreateAlgorithmInstance(AlgorithmNodePacket algorithmNodePacket, string assemblyPath, int loadTimeLimit = 60)
        {
            ...
            var loader = new Loader(algorithmNodePacket.Language, TimeSpan.FromSeconds(loadTimeLimit), names => names.SingleOrAlgorithmTypeName(Config.Get("algorithm-type-name")), WorkerThread);
        ...
        {
            if (algorithmNodePacket is not LiveNodePacket liveJob)
            {
        ...
        public bool Setup(SetupHandlerParameters parameters)
        {
            ...
            void BrokerageOnMessage(object sender, BrokerageMessageEvent args)
            {
            ...
            try
            {
                ...
                var initializeComplete = isolator.ExecuteWithTimeLimit(TimeSpan.FromSeconds(300), () =>
                {
                    try
                    {
                        ...
                        algorithm.SetAvailableDataTypes(GetSecuritydDataFeeds());

                        //Algorithm is live, not backtesting:
                        algorithm.SetLiveMode(true);
                        
                        //Initialize the algorithm's starting date
                        if (!algorithm.SimulationMode)
                        {
                        ...
                        if (algorithm.FutureChainProvider is EmptyFutureChainProvider)
                        {
                            switch (brokerage.Name)
                            {
                                case "Deribit":
                                    algorithm.SetFutureChainProvider(new CachingFutureChainProvider(new LiveDeribitFutureChainProvider()));
                                    break;
                                case "Okex":
                                    algorithm.SetFutureChainProvider(new CachingFutureChainProvider(new LiveOkexFutureChainProvider()));
                                    break;
                                case "Mom":
                                    algorithm.SetFutureChainProvider(new CachingFutureChainProvider(new LiveMomFutureChainProvider((MomBrokerage)brokerage)));
                                    break;
                                default:
                        ...
                        if (algorithm.OptionChainProvider is EmptyOptionChainProvider)
                        {
                            switch (brokerage.Name)
                            {
                                case "Deribit":
                                    algorithm.SetOptionChainProvider(new CachingOptionChainProvider(new LiveDeribitOptionChainProvider()));
                                    break;
                                case "Okex":
                                    algorithm.SetOptionChainProvider(new CachingOptionChainProvider(new LiveOkexOptionChainProvider()));
                                    break;
                                case "Mom":
                                    algorithm.SetOptionChainProvider(new CachingOptionChainProvider(new LiveMomOptionChainProvider((MomBrokerage)brokerage)));
                                    break;
                                default:
                ...
                try
                {
                    ...
                    foreach (var cash in cashBalance)
                    {
                        if (cash.Settled)
                        {
                            ...
                            algorithm.Portfolio.SetCash(cash.Currency, cash.Amount, 0, cash.ConversionRateSecurity);
                        }
                        else
                        {
                            Log.Trace("BrokerageSetupHandler.Setup(): Setting Unsettled " + cash.Currency + " cash to " + cash.Amount);
                            algorithm.Portfolio.SetUnsettledCash(cash.Currency, cash.Amount, 0, cash.ConversionRateSecurity);
                        }
                    }
                }
                catch (Exception err)
                ...
                GetHistoryOrders(algorithm, parameters.ResultHandler, parameters.TransactionHandler, brokerage);

                GetHistoryTrades(algorithm, parameters.ResultHandler, parameters.TransactionHandler, brokerage);
                ...
                try
                {
                    ...
                    // add options first to ensure raw data normalization mode is set on the equity underlying
                    foreach (var holding in holdings.OrderByDescending(x => x.Type))
                    {
                        ...
                        {
                            SecurityHoldingType.Long => security.LongHoldings,
                            SecurityHoldingType.Short => security.ShortHoldings,
                            _ => security.Holdings
                        };
                ...
                if (algorithm.StartingPortfolioValue != 0)
                {
                    StartingPortfolioValue = algorithm.StartingPortfolioValue;
                }
        ...
        private static void AddUnrequestedSecurity(IAlgorithm algorithm, Symbol symbol, Resolution minResolution)
        {
            if (!algorithm.Portfolio.ContainsKey(symbol))
            {
                ...
                else
                {
                    ...
                    algorithm.AddSecurity(symbol.SecurityType, symbol.Value, minResolution, symbol.ID.Market, true, 1.0m, false, symbol.SymbolProperties);
                }
            }
        }
            ...
            HashSet<SecurityType> supportedSecurityTypes, Resolution minResolution)
        {
            ...
            foreach (var order in openOrders.OrderByDescending(x => x.SecurityType))
            {
                ...
                //替换如果有id 就不赋值。权利交给brokerage 在brokerage处如果查到label有
                ...
                //有的会在broker 设置CachedOrderIDs的Order缓存, 但是不知道分配的order id ,此处是回置回去
                brokerage.SetOrderCached(order);

                //初始订单发送
                resultHandler.OrderEvent(new OrderEvent(order, DateTime.UtcNow, OrderFee.Zero));
        ...
        protected void GetHistoryOrders(IAlgorithm algorithm, IResultHandler resultHandler, ITransactionHandler transactionHandler, IBrokerage brokerage)
        {
            var orders = brokerage.GetHistoryOrders();
            foreach (var order in orders)
            {
                foreach (var brokerId in order.order.BrokerId)
                {
                    algorithm.OrderRecords.TryAdd(brokerId, order);
                    resultHandler.OrderEvent(new OrderEvent(order.order, DateTime.UtcNow, OrderFee.Zero));
                }
            }
        }
        ...
        /// <param name="algorithm"></param>
        /// <param name="brokerage"></param>
        protected void GetHistoryTrades(IAlgorithm algorithm, IResultHandler resultHandler, ITransactionHandler transactionHandler, IBrokerage brokerage)
        {
            var trades = brokerage.GetHistoryTrades();
            foreach (var trade in trades)
            {
                algorithm.TradeRecords.TryAdd(trade.TradeId, trade);
                resultHandler.OnTrade(trade);
            }
        }
        ...
        /// </summary>
        public static Dictionary<SecurityType, List<TickType>> GetSecuritydDataFeeds()
        {
            //var dataFeedsConfigString = Config.Get("security-data-feeds");
            var dataFeedsConfigString = _dataFeedsConfigString;

            Dictionary<SecurityType, List<TickType>> config = new Dictionary<SecurityType, List<TickType>>();
            if (dataFeedsConfigString != string.Empty)
            {
                config = JsonConvert.DeserializeObject<Dictionary<SecurityType, List<TickType>>>(dataFeedsConfigString);
            }
