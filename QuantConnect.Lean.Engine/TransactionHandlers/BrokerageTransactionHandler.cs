...
namespace QuantConnect.Lean.Engine.TransactionHandlers
{
    ...
    public class BrokerageTransactionHandler : ITransactionHandler
    {
        ...
        private void ConfigureSerilog()
        {
            ...
            string Logformat = @"[{Timestamp:HH:mm:ss}] [{Level:u3}] " +
                    "[{SourceContext:l}] {Message:lj}{NewLine}{Exception}";
            _logger = new Serilog.LoggerConfiguration()
                .MinimumLevel.Verbose()
                .WriteTo.File("tradelog/tradelog.log", outputTemplate: Logformat, rollingInterval: Serilog.RollingInterval.Day)
                .CreateLogger();
        ...
        public virtual void Initialize(IAlgorithm algorithm, IBrokerage brokerage, IResultHandler resultHandler)
        {
            ...
            _brokerage.OrderStatusChanged += (sender, fill) =>
            {
                ...
                if (algorithm.LiveMode)
                {
                    ...
                    Log.Trace("BrokerageTransactionHandler.OrderStatusChanged() OrderEvent: " + fill + " BrokerId: " + brokerIds);
                    _logger.Information("BrokerageTransactionHandler.OrderStatusChanged() OrderEvent: " + fill + " BrokerId: " + brokerIds);
                }
            ...
            _brokerage.TradeOccurred += (sender, trades) =>
            {
                HandleTradeRecordEvent(trades);
            };

            _brokerage.OrderOccurred += (sender, orders) =>
            {
                HandleOrderRecordEvent(orders);
            };

            _brokerage.QueryEvent += (sender, args) =>
            {
                HandleQueryEvent(args);
            };

            /*
            if (_brokerage.Name == "Multi")
            {
                var mb = _brokerage as QuantConnect.Brokerages.Multi.MultiBrokerage;
                foreach (var sub in mb.SubBrokers)
                {
                    var b = sub.Value as Brokerage;
                    b.OrderStatusChanged += (sender, fill) =>
                    {
                        // log every fill in live mode
                        if (algorithm.LiveMode)
                        {
                    ...
                    b.AccountChanged += (sender, account) =>
                    {
                        HandleAccountChanged(account);
                    };

                    b.OptionPositionAssigned += (sender, fill) =>
                    {
                        HandlePositionAssigned(fill);
                    };
                }
        ...
        public OrderTicket AddOrder(SubmitOrderRequest request)
        {
            ...
            {
                var ret1 = _openOrderTickets.TryAdd(ticket.OrderId, ticket);
                var ret2 = _completeOrderTickets.TryAdd(ticket.OrderId, ticket);
            ...
            else
            {
                ...
                var ret1 = _completeOrderTickets.TryAdd(ticket.OrderId, ticket);
                var ret2 = _completeOrders.TryAdd(order.Id, order);
                Log.Trace($"BrokerageTransactionHandler AddOrder():  OrderId:{ticket.OrderId} _completeOrders.TryAdd {ret2}, _completeOrderTickets.TryAdd {ret1}");
            }
            return ticket;
        }
        ...
        public OrderTicket UpdateOrder(UpdateOrderRequest request)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    _orderRequestQueue.Add(request);
                    order.LastUpdateTime = request.Time; // 订单有更新的时候给order.LastUpdateTime赋值，方便模拟撮合是用该时间戳做比较； writter:fifi
                }
            }
            catch (Exception err)
            {
        ...
        public virtual void ProcessSynchronousEvents()
        {
            ...
            // var currentTimeNewYork = CurrentTimeUtc.ConvertFromUtc(TimeZones.Utc);
            var currentTime = CurrentTimeUtc;
            if (_syncedLiveBrokerageCashToday && currentTime.Date != LastSyncDate)
            {
            ...
            if (_algorithm.LiveMode && !_syncedLiveBrokerageCashToday && currentTime.TimeOfDay >= LiveBrokerageCashSyncTime)
            {
            ...
            const long maxOrdersToKeep = 10000;
            ...
            long max = _completeOrders.Max(x => x.Key);
            long lowestOrderIdToKeep = max - maxOrdersToKeep;
            foreach (var item in _completeOrders.Where(x => x.Key <= lowestOrderIdToKeep))
            {
                //如果订单状态还没结束，不能删除
                if (!item.Value.Status.IsClosed())
                {
                    continue;
                }
                Log.Trace($"BrokerageTransactionHandler.ProcessSynchronousEvents(): delete order({item.Value.Id} {item.Value.BrokerId.FirstOrDefault()} ) from _completeOrders and _completeOrderTickets");
                Order value;
                OrderTicket ticket;
        ...
        private bool PerformCashSync()
        {
            try
            {
                // prevent re-entrance in this method
                if (!Monitor.TryEnter(_performCashSyncReentranceGuard))
                {
                    Log.Trace("BrokerageTransactionHandler.PerformCashSync(): Re-entrant call, cash sync not performed");
                    return false;
                }

                Log.Debug(DateTime.Now + " BrokerageTransactionHandler.PerformCashSync(): Sync cash balance");
                ...
                {
                    if (!_algorithm.Portfolio.CashBook.TryGetValue(balance.Currency, out var _))
                    {
            ...
            Task.Delay(TimeSpan.FromSeconds(10)).ContinueWith(_ =>
            {
                ...
                if (TimeSinceLastFill <= TimeSpan.FromSeconds(20))
                {
                    ...
                    _syncedLiveBrokerageCashToday = false;
                    _failedCashSyncAttempts = 0;
                    Log.Trace("BrokerageTransactionHandler.PerformCashSync(): Unverified cash sync - re-sync required.");
                }
                else
                {
        ...
        private OrderResponse HandleSubmitOrderRequest(SubmitOrderRequest request)
        {
            ...
            {
                Log.Error($"BrokerageTransactionHandler.HandleSubmitOrderRequest(): Unable to add new order, order not processed. {order}");
                return OrderResponse.Error(request, OrderResponseErrorCode.OrderAlreadyExists, $"Cannot process submit request because order with id {order.Id} already exists");
            }
            if (!_completeOrderTickets.TryGetValue(order.Id, out ticket))
            {
                Log.Error($"BrokerageTransactionHandler.HandleSubmitOrderRequest(): Unable to retrieve order ticket with id {order.Id}, order not processed.");
            ...
            try
            {
                _logger.Information("PlaceOrder(): " + order.ToString());
                orderPlaced = _brokerage.PlaceOrder(order);
                _logger.Information("PlaceOrder() Result: " + order.ToString() + $" {orderPlaced}");
            }
            catch (Exception err)
            {
                Log.Error(err);
        ...
        private OrderResponse HandleUpdateOrderRequest(UpdateOrderRequest request)
        {
            ...
            try
            {
                _logger.Information("UpdateOrder(): " + order.ToString());
                orderUpdated = _brokerage.UpdateOrder(order);
                _logger.Information("UpdateOrder() Result: " + order.ToString() + $" {orderUpdated}");
            }
            catch (Exception err)
            {
                Log.Error(err);
        ...
        private OrderResponse HandleCancelOrderRequest(CancelOrderRequest request)
        {
            ...
            try
            {
                _logger.Information("CancelOrder): " + order.ToString());
                orderCanceled = _brokerage.CancelOrder(order);
                _logger.Information("CancelOrder() Result: " + order.ToString() + $" {orderCanceled}");
            }
            catch (Exception err)
            {
                Log.Error(err);
        ...
        private void HandleOrderEvent(OrderEvent fill)
        {
            ...
            {
                if (fill.Status.IsClosed() && _openOrders.TryRemove(fill.OrderId, out var order))
                {
                ...
                if (fill.Status.IsClosed() && _openOrderTickets.TryRemove(fill.OrderId, out var ticket))
                {
                ...
                switch (fill.Status)
                {
                    ...
                    case OrderStatus.Invalid:
                        // append fill message to order tag, for additional information
                        if (!string.IsNullOrWhiteSpace(fill.Message))
                        {
                            if (string.IsNullOrWhiteSpace(order.Tag))
                            {
                                order.Tag = fill.Message;
                ...
                if (fill.Status is OrderStatus.Filled or OrderStatus.PartiallyFilled)
                {
                ...
                order.FillTime = fill.UtcTime;//lean-monitor界面显示修改 writter LHs
                order.FillQuantity = ticket.QuantityFilled;
                order.AverageFillPrice = ticket.AverageFillPrice;
                //lean-monitor界面显示修改  成交金额
                //更新成交信息后再更新订单状态
                order.Status = fill.Status;

                var s = _algorithm.Securities[fill.Symbol];
                order.TradeValue = Math.Abs(order.FillQuantity) * order.Price * s.QuoteCurrency.ConversionRate * s.SymbolProperties.ContractMultiplier;
                //deribit 期货单独处理
                if (order.Symbol.SecurityType == SecurityType.Future && order.Symbol.ID.Market == Market.Deribit)
                {
                    order.TradeValue = Math.Abs(order.FillQuantity);
                }
            }
        ...
        private void HandleAccountChanged(AccountEvent account)
        {
            ...
            {
                // how close are we?
                var delta = _algorithm.Portfolio.UnsettledCashBook[account.CurrencySymbol].Amount - account.CashBalance;
                if (delta != 0)
                {
                    Log.Trace(string.Format("BrokerageTransactionHandler.HandleAccountChanged(): {0} Unsettled Cash Delta: {1}", account.CurrencySymbol, delta));
                }
                ...
                {
                    // override the current cash value so we're always guaranteed to be in sync with the brokerage's push updates
                    _algorithm.Portfolio.UnsettledCashBook[account.CurrencySymbol].SetAmount(account.CashBalance);
                }
            }
        }
        ...
        /// <summary>
        /// process trade append
        /// </summary>
        /// <param name="trades"></param>
        public void HandleTradeRecordEvent(IEnumerable<TradeRecord> trades)
        {
            foreach (var trade in trades)
            {
                _algorithm.TradeRecords.TryAdd(trade.TradeId, trade);
                _resultHandler.OnTrade(trade);
            }
        }
        ...
        public void HandleOrderRecordEvent(IEnumerable<OrderRecord> orders)
        {
            foreach (var order in orders)
            {
                foreach (var brokerId in order.order.BrokerId)
                {
                    if (_algorithm.OrderRecords.ContainsKey(brokerId))
                    {
                        _algorithm.OrderRecords[brokerId] = order;
                    }
                    else
                    {
                        _algorithm.OrderRecords.TryAdd(brokerId, order);
                    }
                }
            }
        }
        ...
        public void Query(AlgorithmQueryType type, string requestId)
        {
            switch (type)
            {
                case AlgorithmQueryType.OpenOrder:
                    _brokerage?.QueryOpenOrders(requestId);
                    break;
                case AlgorithmQueryType.Holding:
                    _brokerage?.QueryAccountHoldings(requestId);
                    break;
                case AlgorithmQueryType.CashBalance:
                    _brokerage?.QueryCashBalance(requestId);
                    break;
            }
        }
        ...
        public void ExchangeQuery(AlgorithmQueryType type, string uId, string requestId)
        {
            switch (type)
            {
                case AlgorithmQueryType.ExchangeOpenOrder:
                    _brokerage?.QueryExchangeOpenOrders(uId, requestId);
                    break;
                case AlgorithmQueryType.ExchangeHolding:
                    _brokerage?.QueryExchangeAccountHoldings(uId, requestId);
                    break;
                case AlgorithmQueryType.ExchangeCashBalance:
                    _brokerage?.QueryExchangeCashBalance(uId, requestId);
                    break;
            }
        }
        ...
        public void HandleQueryEvent(AlgorithmQueryArgs e)
        {
            lock (_queryLock)
            {
                ...
                {
                    case AlgorithmQueryType.OpenOrder:
                        _algorithm.OnQueryOpenOrder(e.Order, e.RequestId);
                        break;
                    case AlgorithmQueryType.Holding:
                        _algorithm.OnQueryAccountHolding(e.Holding, e.RequestId);
                        break;
                    case AlgorithmQueryType.CashBalance:
                        _algorithm.OnQueryCashBalance(e.Balance, e.RequestId);
                        break;

                    case AlgorithmQueryType.ExchangeOpenOrder:
                        _algorithm.OnQueryExchangeOpenOrders(e.Order, e.RequestId, e.IsLast);
                        break;
                    case AlgorithmQueryType.ExchangeHolding:
                        _algorithm.OnQueryExchangeAccountHolding(e.Holding, e.RequestId, e.IsLast);
                        break;
                    case AlgorithmQueryType.ExchangeCashBalance:
                        _algorithm.OnQueryExchangeCashBalance(e.Balance, e.RequestId, e.IsLast);
                        break;
                }
            }
        }
