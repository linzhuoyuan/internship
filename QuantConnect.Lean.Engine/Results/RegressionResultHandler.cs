...
namespace QuantConnect.Lean.Engine.Results
{
    ...
    public class RegressionResultHandler : BacktestingResultHandler
    {
        ...
        /// </summary>
        public override void SetAlgorithm(IAlgorithm algorithm)
        {
            ...
            lock (_sync)
            {
                ...
                while (_preInitializeLines.TryDequeue(out var line))
                {
                    WriteLine(line);
                }
            }
        ...
        public override void OrderEvent(OrderEvent newEvent)
        {
            ...
            lock (_sync)
            {
                ...
                if (newEvent.Symbol.HasUnderlying && Algorithm.Portfolio.TryGetValue(newEvent.Symbol.Underlying, out var underlyingHolding))
                {
            ...
            try
            {
                if (HighFidelityLogging || _lastAlphaRuntimeStatisticsDate != Algorithm.Time.Date)
                {
                    lock (_sync)
                    {
                        ...
                        {
                            if (!_currentAlphaRuntimeStatistics.TryGetValue(kvp.Key, out var value) || value != kvp.Value)
                            {
        ...
        public override void ErrorMessage(string message, string stacktrace = "")
        {
            ...
            stacktrace = string.IsNullOrEmpty(stacktrace) ? stacktrace : Environment.NewLine + stacktrace;
        ...
        public override void RuntimeStatistic(string key, string value)
        {
            try
            {
                if (HighFidelityLogging || _lastRuntimeStatisticsDate != Algorithm.Time.Date)
                {
                    ...
                    if (!_currentRuntimeStatistics.TryGetValue(key, out var existingValue) || existingValue != value)
                    {
