...
namespace QuantConnect.Lean.Engine
{
    ...
    public class Engine
    {
        public static IAlgorithm _algorithm;
        ...
        /// </summary>
        public LeanEngineSystemHandlers SystemHandlers => _systemHandlers;

        /// <summary>
        /// Gets the configured algorithm handlers for this engine instance
        /// </summary>
        public LeanEngineAlgorithmHandlers AlgorithmHandlers => _algorithmHandlers;
        ...
        public void Run(AlgorithmNodePacket job, AlgorithmManager manager, string assemblyPath, WorkerThread? workerThread = null)
        {
            ...
            try
            {
                ...
                Thread? threadOptionPriceResults = null;
                ...
                _algorithmHandlers.Results.IsPushOptionPrice = Config.GetBool("push-option-price", false);
                /*if (_algorithmHandlers.Results.IsPushOptionPrice)
                {
                    threadOptionPriceResults = new Thread(_algorithmHandlers.Results.RunOptionPrice, 0) { IsBackground = true, Name = "OptionPrice Result Thread" };
                    threadOptionPriceResults.Start();
                }*/
                ...
                try
                {
                    ...
                    brokerage = _algorithmHandlers.Setup.CreateBrokerage(job, algorithm, out var factory);
                    ...
                    //SymbolBonusDatabase.FromDataFolder();
                    ...
                    if (historyProvider is BrokerageHistoryProvider provider)
                    {
                        provider.SetBrokerage(brokerage);
                    }
                    else
                    {
                        brokerage.Connect();
                    ...
                    if (!initializeComplete || algorithm.ErrorMessages.Count > 0 || _algorithmHandlers.Setup.Errors.Count > 0)
                    {
                        ...
                        errorMessage += string.Join(",", _algorithmHandlers.Setup.Errors.Select(e =>
                        {
                            ...
                            {
                                var err = _exceptionInterpreter.Value.Interpret(
                                    e.InnerException, _exceptionInterpreter.Value);
                        ...
                        _systemHandlers.Api.SetAlgorithmStatus(
                            job.AlgorithmId, AlgorithmStatus.RuntimeError, errorMessage);
                    }
                }
                catch (Exception err)
                {
                    ...
                    _systemHandlers.Api.SetAlgorithmStatus(
                        job.AlgorithmId, AlgorithmStatus.RuntimeError, runtimeMessage);
                }
                ...
                if (initializeComplete)
                {
                    ...
                    algorithm.SetLocked();
                    algorithm.SetSupportAutoOpenClose(Config.GetBool("auto-openclose"));
                    ...
                    try
                    {
                        ...
                        _algorithm.WaitForLiveStart(isolator);

                        // Execute the Algorithm Code:
                        var complete = isolator.ExecuteWithTimeLimit(
                            _algorithmHandlers.Setup.MaximumRuntime,
                            algorithmManager.TimeLoopWithinLimits,
                            () =>
                            {
                                try
                                {
                                    ...
                                    //把results 传进去为了期权敏感性矩阵发送行情
                                    synchronizer.ResultHandler = _algorithmHandlers.Results;
                                    algorithm.ExitTokenSource = isolator.CancellationTokenSource;
                        ...
                        {
                            Log.Error($"The algorithm exited with code (int): {algorithm.ExitCode}");
                    ...
                    catch (Exception err)
                    {
                        ...
                        algorithmManager.ExitCode = System.Runtime.InteropServices.Marshal.GetHRForException(err) & ((1 << 16) - 1);
                        Log.Error($"The algorithm manager console exited with code (int): {algorithmManager.ExitCode}");
                    }
                    ...
                    try
                    {
                        ...
                        var orders = _algorithmHandlers.Transactions.Orders.ToDictionary(t => t.Key.ToString(), t => t.Value);
                        ...
                        algorithm.Portfolio.SummaryHoldings(holdings);
                        ...
                        try
                        {
                            ...
                            )
                            {
                                ...
                                banner.Add("Net Profit", "$" + (algorithm.Portfolio.TotalProfit - algorithm.Portfolio.TotalFees).ToString("N2"));

                                banner.Add("Option Value", "$" + algorithm.Portfolio.GetMarketValue(holdings, QuantConnect.SecurityType.Option).ToString("N2"));
                                banner.Add("Equity Value", "$" + algorithm.Portfolio.GetMarketValue(holdings, QuantConnect.SecurityType.Equity).ToString("N2"));
                                banner.Add("Future PnL", "$" + algorithm.Portfolio.GetTotalFutureUnrealisedProfit(holdings, QuantConnect.SecurityType.Future).ToString("N2"));


                                banner.Add("Return", netReturn.ToString("P"));
                                banner.Add("Equity", "$" + algorithm.Portfolio.TotalPortfolioValue.ToString("N2"));

                                banner.Add("Available Margin", "$" + algorithm.Portfolio.GetMarginRemaining(0m).ToString("N2"));
                                banner.Add("Used Margin", "$" + algorithm.Portfolio.TotalMarginUsed.ToString("N2"));
                                banner.Add("Unsettled Cash", "$" + algorithm.Portfolio.UnsettledCash.ToString("N2"));
                            }
                        }
                        catch (Exception err)
                        {
                        ...
                        _algorithmHandlers.Results.SendFinalResult(
                            job,
                            orders,
                ...
                if (threadResults != null && threadResults.IsAlive) threadResults.Interrupt();
                if (threadAlphas != null && threadAlphas.IsAlive) threadAlphas.Interrupt();
                ...
                //Log.Trace($"交易完成后账户信息: {Environment.NewLine}{algorithm.Portfolio.CashBook}");
                //Log.Trace("使用保证金" + $"{algorithm.Portfolio.TotalMarginUsed}"+"保证金余额:"+$"{algorithm.Portfolio.MarginRemaining}");
            }
            catch (Exception err)
            {
