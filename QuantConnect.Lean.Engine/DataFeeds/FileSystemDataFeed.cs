...
namespace QuantConnect.Lean.Engine.DataFeeds
{
    ...
    public class FileSystemDataFeed : IDataFeed
    {
        ...
        /// <summary>
        /// Flag indicating the handler thread is completely finished and ready to dispose.
        /// </summary>
        ...
        private static void ScheduleEnumerator(
            Subscription subscription,
            IEnumerator<BaseData> enumerator,
            EnqueueableEnumerator<SubscriptionData> enqueueable,
            int lowerThreshold,
            int upperThreshold,
            SecurityExchangeHours exchangeHours,
            int firstLoopCount = 5)
        {
            void Produce()
            {
                var count = 0;
                while (enumerator.MoveNext())
                {
                    ...
                    var subscriptionData = SubscriptionData.Create(
        ...
        private Subscription CreateUniverseSubscription(SubscriptionRequest request)
        {
            ...
            // add this enumerator to our exchange
            ScheduleEnumerator(
                subscription, enumerator, enqueueable,
                lowerThreshold, upperThreshold, request.Security.Exchange.Hours, firstLoopCount);
        ...
        private ISubscriptionEnumeratorFactory GetEnumeratorFactory(SubscriptionRequest request)
        {
            ...
            {
                if (request.Universe is ITimeTriggeredUniverse requestUniverse)
                {
                    if (requestUniverse is UserDefinedUniverse universe)
                    {
                    ...
                    return new TimeTriggeredUniverseSubscriptionEnumeratorFactory(requestUniverse, MarketHoursDatabase.FromDataFolder());
                }
        ...
        private IEnumerator<BaseData> ConfigureEnumerator(SubscriptionRequest request, bool aggregate, IEnumerator<BaseData> enumerator)
        {
            ...
            //Tick数据导入改动:新建Option Enumerator
            ...
            if (request.Configuration.FillDataForward)
            {
                ...
                // Tick数据导入改动:判断是否为期权链的加载；
                if (request.Configuration.Resolution == Resolution.Tick)
                {
                    enumerator = new OptionChainTickFillForwardEnumerator(
                        enumerator,
                        request.Security.Exchange,
                        fillForwardResolution,
                        request.Security.IsExtendedMarketHours,
                        request.EndTimeLocal,
                        request.Configuration.Resolution.ToTimeSpan(),
        ...
        {
            var lowerThresholdConfig = Config.Get("lower-threshold-data-feeds");
            if (!string.IsNullOrEmpty(lowerThresholdConfig))
            {
                var config = new Dictionary<Resolution, int>();
                if (lowerThresholdConfig != string.Empty)
                {
                    config = JsonConvert.DeserializeObject<Dictionary<Resolution, int>>(lowerThresholdConfig);
                }
        ...
        {
            var upperThresholdConfig = Config.Get("upper-threshold-data-feeds");
            if (!string.IsNullOrEmpty(upperThresholdConfig))
            {
                var config = new Dictionary<Resolution, int>();
                if (upperThresholdConfig != string.Empty)
                {
                    config = JsonConvert.DeserializeObject<Dictionary<Resolution, int>>(upperThresholdConfig);
                }
                return config[resolution];
            }
