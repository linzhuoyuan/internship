...
namespace QuantConnect.Lean.Engine.DataFeeds
{
    ...
    public class UniverseSelection
    {
            ...
            IDataProvider dataProvider = null)
        {
            ...
            _dataProvider = dataProvider ?? new DefaultDataProvider();
        }
        ...
        public SecurityChanges ApplyUniverseSelection(Universe universe, DateTime dateTimeUtc, BaseDataCollection universeData)
        {
            ...
            if (universe is FineFundamentalFilteredUniverse fineFiltered)
            {
                ...
                if (!ReferenceEquals(selectSymbolsResult, Universe.Unchanged))
                {
                    ...
                    foreach (var fine in fineCollection.Data.OfType<FineFundamental>())
                    {
                        ...
                        if (coarseData.TryGetValue(fine.Symbol, out var coarse))
                        {
            ...
            if (!_pendingSecurityAdditions.TryGetValue(dateTimeUtc, out var pendingAdditions))
            {
            ...
            foreach (var symbol in selections)
            {
                ...
                if (!pendingAdditions.TryGetValue(symbol, out var security) && !_algorithm.Securities.TryGetValue(symbol, out security))
                {
