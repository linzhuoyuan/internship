...
namespace QuantConnect.Lean.Engine.DataFeeds
{
    ...
    public class DataManager : IAlgorithmSubscriptionManager, IDataFeedSubscriptionManager, IDataManager
    {
            ...
            bool liveMode)
        {
            ...
            var liveStart = algorithm.SimulationMode ? algorithm.StartDate.ConvertToUtc(algorithm.TimeZone) : DateTime.UtcNow;
            // wire ourselves up to receive notifications when universes are added/removed
            algorithm.UniverseManager.CollectionChanged += (_, args) =>
            {
                switch (args.Action)
                {
                    case NotifyCollectionChangedAction.Add:
                        foreach (var universe in args.NewItems.OfType<Universe>())
                        {
                            ...
                            if (!algorithm.Securities.TryGetValue(config.Symbol, out var security))
                            {
        ...
        private void RemoveSubscriptionDataConfig(Subscription subscription)
        {
            if (subscription.RemovedFromUniverse.Value
                && _subscriptionManagerSubscriptions.TryRemove(subscription.Configuration, out var config))
            {
            ...
            )
        {
            return Add(symbol, resolution, fillForward, extendedMarketHours, isFilteredSubscription, isInternalFeed, isCustomData,
                new List<Tuple<Type, TickType>> { new(dataType, LeanData.GetCommonTickTypeForCommonDataTypes(dataType, symbol.SecurityType))}, dataNormalizationMode)
                .First();
        }
            ...
            )
        {
            ...
            if (symbol.ID.SecurityType is SecurityType.Option or SecurityType.Future)
            {
            ...
            )
        {
            if (isCanonical)
            {
                return new List<Tuple<Type, TickType>> { new(typeof(ZipEntryName), TickType.Quote) };
            }
        ...
        public void Remove(SubscriptionDataConfig subscription)
        {
