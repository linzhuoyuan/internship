...
namespace QuantConnect.Lean.Engine.DataFeeds
{
    ...
    public class BaseDataExchange
    {
        ...
        private readonly ConcurrentDictionary<Symbol, EnumeratorHandler> _enumerators;
        ...
        /// <summary>
        /// Sets the specified handler function to handle data for the handler's symbol
        /// </summary>
        /// <param name="handler">The handler to use when this symbol's data is encountered</param>
        ...
        /// <summary>
        /// Adds the specified handler function to handle data for the handler's symbol
        /// </summary>
        /// <param name="symbol">The symbol whose data is to be handled</param>
        /// <param name="handler">The handler to use when this symbol's data is encountered</param>
        /// <returns>An identifier that can be used to remove this handler</returns>
        ...
        {
            return _dataHandlers.TryRemove(symbol, out _);
        }
        ...
        {
            if (_enumerators.TryRemove(symbol, out var handler))
            {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    foreach (var kvp in _enumerators)
                    {
                        ...
                        if (_dataHandlers.TryGetValue(enumerator.Current.Symbol, out var dataHandler))
                        {
                            handled = true;
                            if (enumerator.Current.Symbol.Value == "BNBBUSD")
                            {
