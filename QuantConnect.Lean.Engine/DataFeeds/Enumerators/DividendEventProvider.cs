    ...
    public class DividendEventProvider : ITradableDateEventProvider
    {
        ...
        public IEnumerable<BaseData> GetEvents(NewTradableDateEventArgs eventArgs)
        {
                ...
                && _mapFile.HasData(eventArgs.Date))
            {
                ...
                {
                    if (AblSymbolDatabase.IsChinaStocks(_config.Symbol) || AblSymbolDatabase.IsChinaEtf(_config.Symbol))
                    {
                        var info = AblSymbolDatabase.GetDividend(_config.Symbol, eventArgs.Date);
                        if (info != null)
                        {
                            var dividend = new Dividend(_config.symbol, eventArgs.Date, info.DividendCash, close);
                            dividend.ExDividend = info.ExDividendDate == eventArgs.Date;
                            dividend.PayDividend = info.PayDate == eventArgs.Date;
                ...
                if (_factorFile.HasDividendEventOnNextTradingDay(eventArgs.Date, out var priceFactorRatio))
                {
