    ...
    public class SplitEventProvider : ITradableDateEventProvider
    {
        ...
        public IEnumerable<BaseData> GetEvents(NewTradableDateEventArgs eventArgs)
        {
                ...
                && _mapFile.HasData(eventArgs.Date))
            {
                if (_config.Symbol.IsChinaMarket())
                {
                    if (AblSymbolDatabase.IsChinaStocks(_config.Symbol) || AblSymbolDatabase.IsChinaEtf(_config.Symbol))
                    {
                        var close = AuxiliaryDataEnumerator.GetRawClose(
                            ...
                            _config);
                        var info = AblSymbolDatabase.GetSplit(_config.Symbol, eventArgs.Date);
                        if (info != null)
                        {
                            var split = new Split(
                                _config.Symbol, 
                                eventArgs.Date, close, 
                                info.SplitFactor,
                ...
                if (_factorFile.HasSplitEventOnNextTradingDay(eventArgs.Date, out var splitFactor))
                {
