...
namespace QuantConnect.Lean.Engine.DataFeeds.Enumerators.Factories
{
    ...
    public class OptionChainUniverseSubscriptionEnumeratorFactory : ISubscriptionEnumeratorFactory
    {
        ...
        public IEnumerator<BaseData> CreateEnumerator(SubscriptionRequest request, IDataProvider dataProvider)
        {
            ...
            else
            {
                ...
                var enumerators = GetSubscriptionConfigurations(request)
                    .Select(c =>
                    {
                        return new SubscriptionRequest(request, configuration: c);
                    })
                    .Select(sr =>
                    {
                        return _enumeratorConfigurator(request, factory.CreateEnumerator(sr, dataProvider));
                    });
        ...
        private IEnumerable<SubscriptionDataConfig> GetSubscriptionConfigurations(SubscriptionRequest request)
        {
            ...
            //Tick数据导入改动:放开Resolution不能为Tick限制;writter:wh
            // Making sure data is non-tick
            //var resolution = config.Resolution == Resolution.Tick ? Resolution.Second : config.Resolution;
            var resolution = config.Resolution;

            //Tick数据导入改动:针对Resolution为Tick时,objectType为Tick;writter:wh
            var objectType = typeof(TradeBar);
            if (resolution == Resolution.Tick)
            {
                objectType = typeof(Tick);
            }

            var configurations = new List<SubscriptionDataConfig>
            {
                //Tick数据导入改动:objectType取值由typeof(TradeBar)改为objectType变量;writter:wh
                ...
                new SubscriptionDataConfig(config, resolution: resolution, fillForward: true, symbol: underlying, objectType: objectType, tickType: TickType.Trade),
            };
