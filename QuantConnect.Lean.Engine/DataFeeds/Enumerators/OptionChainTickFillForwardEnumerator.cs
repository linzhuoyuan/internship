...
namespace QuantConnect.Lean.Engine.DataFeeds.Enumerators
{
    ...
    /// </summary>
    public class OptionChainTickFillForwardEnumerator : IEnumerator<BaseData>
    {
        ...
        public OptionChainTickFillForwardEnumerator(IEnumerator<BaseData> enumerator,
        ...
        public bool MoveNext()
        {
            ...
            if (_current != null && _current.dataType != MarketDataType.Auxiliary)
            {
            ...
            if (!_isFillingForward)
            {
                ...
                if (!_enumerator.MoveNext())
                {
                    ...
                    var endOfSubscription = (_current ?? _previous).Clone(true);
                    endOfSubscription.time = _subscriptionEndTime.RoundDownInTimeZone(
                        _dataResolution, Exchange.TimeZone, _dataTimeZone);
            ...
            {
                _current = underlyingCurrent;
                if (_current is Delisting delisting && delisting.Type == DelistingType.Delisted)
                {
                    _delistedTime = delisting.endTime;
                }
                return true;
            }
            // Tick数据导入改动：使得标的不做实际填充；
            // writer:fifi
            // 判断baseDataCollection是否为期权链
            var flag = false;
            if (underlyingCurrent is BaseDataCollection baseDataCollection)
			{
				if (baseDataCollection.data.Count > 1 && baseDataCollection.symbol.value.Contains("?"))
				{
					flag = true;
				}

			}
			if (flag && RequiresFillForwardData(_fillForwardResolution.Value, _previous, underlyingCurrent, out fillForward))
			{
