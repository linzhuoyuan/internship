    ...
    public class MappingEventProvider : ITradableDateEventProvider
    {
        ...
        public IEnumerable<BaseData> GetEvents(NewTradableDateEventArgs eventArgs)
        {
                ...
                && _mapFile.HasData(eventArgs.Date))
            {
                ...
                else if (eventArgs.Symbol.ID.SecurityType == SecurityType.Option && eventArgs.Symbol.ID.ContractId != null &&
                    eventArgs.Symbol.ID.Market == Market.SSE)
                {
                    SymbolBonus symbolBonus = SymbolBonusDatabase.GetSymbolBonus(eventArgs.Symbol.ID.ContractId, eventArgs.Date.ToString(DateFormat.EightCharacter));
                    if (symbolBonus != null)
                    {
                        var changed = new SymbolBonusEvent(
                            _config.Symbol,
                            symbolBonus.StrikePrice,
                            symbolBonus.ContractMultiplier,
                            eventArgs.Date);
                        yield return changed;
                    }
                }
