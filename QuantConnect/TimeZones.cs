...
namespace QuantConnect
{
    ...
    {
        private static DateTime ToUnspecified(DateTime utcTime)
        {
            return DateTime.SpecifyKind(utcTime, DateTimeKind.Unspecified);
        }

        public static DateTimeOffset GetDateTimeOffset(DateTimeZone zone, DateTime utcTime)
        {
            var offset = zone.GetUtcOffset(Instant.FromDateTimeUtc(utcTime)).ToTimeSpan();
            return new DateTimeOffset(ToUnspecified(utcTime).Add(offset), offset);
        }

        public static DateTimeZone GetDateTimeZone(string name)
        {
            return DateTimeZoneProviders.Tzdb[name];
        }

        public static DateTimeOffset GetTimeZoneNow(DateTimeZone zone)
        {
            return GetDateTimeOffset(zone, DateTime.UtcNow);
        }

        public static DateTimeOffset GetZoneToday(DateTimeZone zone)
        {
            var now = GetTimeZoneNow(zone);
            return new DateTimeOffset(now.Date, now.Offset);
        }

        public static Offset GetOffset(DateTimeZone zone, DateTime utcTime)
        {
            return zone.GetUtcOffset(Instant.FromDateTimeUtc(utcTime));
        }
        ...
        public static readonly DateTimeZone WesternStandard = DateTimeZoneProviders.Tzdb["Pacific/Pitcairn"];
