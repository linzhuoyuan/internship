...
namespace QuantConnect.Orders
{
    ...
    public class OrderJsonConverter : JsonConverter
    {
        ...
        public static Order CreateOrderFromJObject(JObject jObject)
        {
            ...
            order.Id = jObject["Id"].Value<long>();
            order.Status = (OrderStatus) jObject["Status"].Value<int>();
            order.Time = jObject["Time"].Value<DateTime>();
            //lean-monitor界面显示修改
            //writer LH
            if (jObject.TryGetValue("FillTime", out var fillTime))
            {
                order.FillTime = fillTime.Value<DateTime>();
            }
            if (jObject.TryGetValue("TradeValue", out var tradeValue))
            {
                order.TradeValue = tradeValue.Value<decimal>();
            }
            if (jObject.TryGetValue("TimeZoneTime", out var timeZoneTime))
            {
                order.TimeZoneTime = timeZoneTime.Value<DateTime>();
            }
            if (jObject.TryGetValue("Offset", out var offset))
            {
                order.Offset = (OrderOffset)jObject["Offset"].Value<int>();
            }
            ...
            var suppliedMarket = jObject.SelectTokens("Symbol.ID.Market").ToArray();
