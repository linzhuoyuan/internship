...
namespace QuantConnect.Securities
{
    ...
    {
        public static readonly HashSet<string> CryptoUsdToken = new()
        {
            "USDT",
            "USDC",
            "BUSD",
        ...
        public event EventHandler<CashBookEventArgs> Updated;
        ...
        public string AccountCurrency
        {
            ...
            set
            {
                ...
                if (!_accountCurrency.IsNullOrEmpty()
                    && TryGetValue(_accountCurrency, out var accountCurrency))
                {
        ...
        public void Add(string symbol, decimal quantity, decimal conversionRate, string currencySymbol)
        {
            var cash = new Cash(symbol, quantity, conversionRate, currencySymbol);
            Add(symbol, cash);
        }
            ...
            ISecurityService securityService)
        {
            ...
            foreach (var kvp in _currencies)
            {
                ...
                var accountCurrency = CryptoUsdToken.Contains(cash.CurrencySymbol)
                    ? cash.CurrencySymbol
                    : AccountCurrency;
        ...
        public decimal Convert(decimal sourceQuantity, string sourceCurrency, string destinationCurrency)
        {
            ...
            if (CryptoUsdToken.Contains(sourceCurrency) && destinationCurrency == Currencies.USD)
            {
        ...
        public void Add(string symbol, Cash value)
        {
            ...
            OnUpdate(alreadyExisted ? UpdateType.Updated : UpdateType.Added, new CashEventArgs(value.Symbol));
        }
        ...
        {
            _currencies.Clear();
            OnUpdate(UpdateType.Removed, new CashEventArgs(""));
        }
        ...
        {
            var removed = _currencies.TryRemove(symbol, out var cash);
            if (!removed)
            {
                if (!calledInternally)
                {
            ...
            else
            {
                ...
                if (!calledInternally)
                {
                    ...
                    OnUpdate(UpdateType.Removed, new CashEventArgs(cash.Symbol));
                }
            }
            return removed;
        }

        private void OnCashUpdate(object sender, CashEventArgs eventArgs)
        {
            OnUpdate(UpdateType.Updated, eventArgs);
        }

        private void OnUpdate(UpdateType updateType, CashEventArgs eventArgs)
        {
            //Updated?.Invoke(this, updateType);
            Updated?.Invoke(this, new CashBookEventArgs(updateType, eventArgs));
        }
        ...
        /// 
        /// </summary>
        public class CashBookEventArgs : EventArgs
        {
            /// <summary>
            /// 
            /// </summary>
            ...
            /// 
            /// </summary>
            public CashEventArgs CashEventArgs { get; set; }
            ...
            public CashBookEventArgs(UpdateType type, CashEventArgs cash)
            {
                UpdateType = type;
                CashEventArgs = cash;
            }
