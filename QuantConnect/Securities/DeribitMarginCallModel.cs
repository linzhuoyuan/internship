...
namespace QuantConnect.Securities
{
    ...
    /// 
    /// </summary>
    public class DeribitMarginCallModel : IMarginCallModel
    {
        /// <summary>
        ...
        public DeribitMarginCallModel(SecurityPortfolioManager portfolio, IOrderProperties defaultOrderProperties)
        {
            Portfolio = portfolio;
        ...
        /// 剩余保证金 < 维持保证金*1.2 预警
        /// 剩余保证金 <= 维持保证金 策略停止 发布警告
        /// 剩余保证金 < 维持保证金*1.2 预警这个 最好我能在策略里面捕捉到
        /// </summary>
        ...
        public List<SubmitOrderRequest> GetMarginCallOrders(out bool issueMarginCallWarning)
        {
            ...
            var btc_orders = GetMarginCallOrdersForCurrency(out var btc_waring, "BTC");
            marginCallOrders.AddRange(btc_orders);
            var eth_orders = GetMarginCallOrdersForCurrency(out var eth_waring, "BTC");
            marginCallOrders.AddRange(eth_orders);

            issueMarginCallWarning = btc_waring || eth_waring;
            return marginCallOrders;
        }


        private List<SubmitOrderRequest> GetMarginCallOrdersForCurrency(out bool issueMarginCallWarning,string currency)
        {
            issueMarginCallWarning = false;

            var totalMarginUsed = Portfolio.TotalMarginUsedForCurrency(currency);
            ...
            var totalPortfolioValue = Portfolio.GetTotalPortfolioValueForCurrency(currency);

            var marginRemaining = Portfolio.GetMarginRemainingForCurrency(currency,totalPortfolioValue);

            // issue a margin warning when we're down to 5% margin remaining
            if (marginRemaining < totalMarginUsed * 1.2m)
            {
