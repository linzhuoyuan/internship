...
namespace QuantConnect.Securities.Future
{
    ...
    public static class FuturesExpiryUtilityFunctions
    {
        ...
        /// <summary>
        /// Method to retrieve the 3rd Friday of the given month ，if it is a holiday ,return the next tradeday
        /// </summary>
        /// <param name="time">Date from the given month</param>
        /// <returns>3rd Friday of given month</returns>
        public static DateTime ThirdFridayChina(DateTime time) => NthFridayChina(time, 3);
        ...
        /// <summary>
        /// Method to retrieve the Nth Friday of the given month, if it is a holiday ,return the next tradeday
        /// </summary>
        /// <param name="time">Date from the given month</param>
        /// <param name="n">The order of the Friday in the period</param>
        /// <returns>Nth Friday of given month</returns>
        //添加中国获取第n个周五的方法，如果周五是节假日，顺延到下个工作日 writter ：lh
        public static DateTime NthFridayChina(DateTime time, int n)
        {
            if (n < 1 || n > 5)
            {
                throw new ArgumentOutOfRangeException($"'n' lower than 1 or greater than 5");
            ...
            var daysInMonth = DateTime.DaysInMonth(time.Year, time.Month);
            var thirdFriday = (from day in Enumerable.Range(1, daysInMonth)
                    where new DateTime(time.Year, time.Month, day).DayOfWeek == DayOfWeek.Friday
                    select new DateTime(time.Year, time.Month, day)).ElementAt(n - 1);

            while (!NotHolidayChina(thirdFriday))
            {
        ...
        /// <summary>
        /// Method to check whether a given time is holiday or not( for china)
        /// </summary>
        /// <param name="time">The DateTime for consideration</param>
        /// <returns>True if the time is not a holidays, otherwise returns false</returns>
          //添加中国是否是节假日的方法  writter ：lh
        public static bool NotHolidayChina(DateTime time)
        {
            return time.IsCommonBusinessDay() && !ChinaHoliday.Dates.Contains(time);
        }
