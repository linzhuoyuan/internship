...
namespace QuantConnect.Securities.Future
{
    ...
    public class FuturesExpiryFunctions
    {
        ...
        public static Func<DateTime, DateTime> FuturesExpiryFunction(string symbol)
        {
            ...
            if (symbol.ToUpper().Contains("PERPETUAL"))
            {
                return date => date.AddDays(30);
            }
        ...
        public static Dictionary<string, Func<DateTime, DateTime>> FuturesExpiryDictionary = new Dictionary<string, Func<DateTime, DateTime>>()
        {
            ...
            //添加中国股指期货的交割日的计算 writter: lh  ----begin---
            // 上证50股指期货 (IH)
            {Futures.Indices.SZ50, (time =>
                {
                    // Trading can occur up to 9:30 a.m. BeiJing Time on the 3rd Friday of the contract month
                    var thirdFriday = FuturesExpiryUtilityFunctions.ThirdFridayChina(time);
                    return thirdFriday.Add(new TimeSpan(01,30,0));
                })
            },
            // 沪深300股指期货 (IF)
            {Futures.Indices.HS300, (time =>
                {
                    // Trading can occur up to 9:30 a.m. BeiJing Time on the 3rd Friday of the contract month
                    var thirdFriday = FuturesExpiryUtilityFunctions.ThirdFridayChina(time);
                    return thirdFriday.Add(new TimeSpan(01,30,0));
            ...
            //添加中国股指期货的交割日的计算 writter: lh  ----end---
