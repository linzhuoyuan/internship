...
namespace QuantConnect.Securities
{
    ...
    public class MarketHoursDatabase
    {
        ...
        private static readonly object DataFolderMarketHoursDatabaseLock = new();
        internal readonly IDictionary<SecurityDatabaseKey, Entry> entries;

        internal void Merge(MarketHoursDatabase other)
        {
        ...
        public static void UseAlwaysOpen()
        {
            _dataFolderMarketHoursDatabase = AlwaysOpen;
        }
        ...
        {
            entries = exchangeHours.ToDictionary();
        }
        ...
        {
            dataTimeZone ??= exchangeHours.TimeZone;
        ...
        public virtual Entry GetEntry(string market, string symbol, SecurityType securityType)
        {
            var list = new[]
            {
                new SecurityDatabaseKey(market, symbol, securityType),
                new SecurityDatabaseKey(market, null, securityType),
                new SecurityDatabaseKey(market, null, SecurityType.Base)
            };
            ...
            if (entry == null)
            {
                var keys = string.Join(", ", entries.Keys);
                Log.Error($"MarketHoursDatabase.GetExchangeHours(): Unable to locate exchange hours for {list[0]}.Available keys: {keys}");
                // there was nothing that really matched exactly... what should we do here?
                throw new ArgumentException("Unable to locate exchange hours for " + list[0]);
            }
