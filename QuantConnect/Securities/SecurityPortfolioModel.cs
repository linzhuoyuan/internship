...
namespace QuantConnect.Securities
{
    ...
    public class SecurityPortfolioModel : ISecurityPortfolioModel
    {
        ...
        {
            //下单接口已做判断，故在此不做判断
            //if (security.Symbol.IsChinaMarket() && portfolio.Transactions.LiveMode)
            //{
            //    ProcessChineseFill(portfolio, security, fill);
            //    return;
            //}
            ...
            if (fill.Offset != OrderOffset.None)
            {
                ProcessOffsetFill(portfolio, security, fill);
                return;
            }
            ...
            var quantityHoldings = security.Holdings.Quantity;
            ...
            try
            {
                ...
                // we don't adjust funds for futures and CFDs: it is zero upfront payment derivative (margin applies though)
                ...
                if (security.Type is SecurityType.Forex or SecurityType.Crypto)
                {
                ...
                if (closedPosition)
                {
                    ...
                    if (security.Type is SecurityType.Future or SecurityType.Cfd)
                    {
                    ...
                    security.Holdings.ClosePosition(fill, feeInAccountCurrency);
                }
                else
                {
                    security.Holdings.OpenPosition(fill, feeInAccountCurrency);
                }
            ...
            security.Holdings.AveragePrice = averageHoldingsPrice;
        }
        ...
        /// <param name="security">The fill's security</param>
        /// <param name="fill">The order event fill object to be applied</param>
        public void ProcessOffsetFill(SecurityPortfolioManager portfolio, Security security, OrderEvent fill)
        {
            if (fill.Direction == OrderDirection.Hold)
            {
                Log.Error("PortfolioModel.ProcessOffsetFill():  fill.Direction == OrderDirection.Hold ,Order with id " + fill.OrderId);
                return;
            }
            if (fill.Offset == OrderOffset.None)
            {
                Log.Error("PortfolioModel.ProcessOffsetFill():  fill.Offset == OrderOffset.None ,Order with id " + fill.OrderId);
                return;
            }

            SecurityHolding holding = null;
            if ((fill.Direction == OrderDirection.Buy && fill.Offset == OrderOffset.Open) ||
                (fill.Direction == OrderDirection.Sell && fill.Offset == OrderOffset.Close))
            {
                holding = security.LongHoldings;
            }
            if ((fill.Direction == OrderDirection.Buy && fill.Offset == OrderOffset.Close) ||
                (fill.Direction == OrderDirection.Sell && fill.Offset == OrderOffset.Open))
            {
                holding = security.ShortHoldings;
            }
            if (holding == null)
            {
                Log.Error("PortfolioModel.ProcessOffsetFill(): holding is null, OrderId: " + fill.OrderId);
                return;
            }
            if (holding.AbsoluteQuantity < Math.Abs(fill.FillQuantity) && fill.Offset == OrderOffset.Close)
            {
                Log.Error($"PortfolioModel.ProcessOffsetFill(): holding.Quantity:{holding.Quantity} fill.FillQuantity:{fill.FillQuantity} && order.Offset == OrderOffset.Close, OrderId: {fill.OrderId}");
                return;
            }
            ...
            //Get the required information from the vehicle this order will affect
            var isLong = holding.HoldingType == SecurityHoldingType.Long;
            var isShort = holding.HoldingType == SecurityHoldingType.Short;
            //Make local decimals to avoid any rounding errors from int multiplication
            var quantityHoldings = holding.Quantity;
            var averageHoldingsPrice = holding.AveragePrice;
            ...
            try
            {
                    ...
                    && fill.OrderFee.Value.Amount != 0)
                {
                    var feeThisOrder = fill.OrderFee.Value;
                    feeInAccountCurrency = portfolio.CashBook.ConvertToAccountCurrency(feeThisOrder).Amount;
                    holding.AddNewFee(feeInAccountCurrency);
                    portfolio.CashBook[feeThisOrder.Currency].AddAmount(-feeThisOrder.Amount);
                }
                ...
                // did we close or open a position further?
                var closedPosition = fill.Offset == OrderOffset.Close;
                ...
                if (closedPosition)
                {
                    ...
                    // closed sale value = quantity closed * fill price       BUYs are deemed negative cash flow
                    // cost = quantity closed * average holdings price        SELLS are deemed positive cash flow
                    var absoluteQuantityClosed = fill.AbsoluteFillQuantity;
                    var closedSaleValueInQuoteCurrency = fill.FillPrice * absoluteQuantityClosed;
                    var closedCost = averageHoldingsPrice * absoluteQuantityClosed;
                    var lastTradeProfit = (closedSaleValueInQuoteCurrency - closedCost)
                        * security.SymbolProperties.ContractMultiplier;
                    var lastTradeProfitInAccountCurrency = lastTradeProfit * security.QuoteCurrency.ConversionRate;
                    ...
                    //Update Vehicle Profit Tracking:
                    holding.AddNewProfit(lastTradeProfitInAccountCurrency);
                    holding.SetLastTradeProfit(lastTradeProfitInAccountCurrency);
                    portfolio.AddTransactionRecord(security.LocalTime.ConvertToUtc(
                        security.Exchange.TimeZone),
                        lastTradeProfitInAccountCurrency - 2 * feeInAccountCurrency);
                    holding.ClosePosition(fill, feeInAccountCurrency);
                }
                else
                {
                    holding.OpenPosition(fill, feeInAccountCurrency);
                }
                //UPDATE HOLDINGS QUANTITY, AVG PRICE:
                //Currently NO holdings. The order is ALL our holdings.

                //openbuy +1 , open sell -1, closebuy +1, closesell -1 

                if (quantityHoldings == 0)
                {
                    //First transaction just subtract order from cash and set our holdings:
                    if (fill.Offset == OrderOffset.Open)
                    {
                        var factor = fill.Direction == OrderDirection.Buy ? 1 : -1;
                        averageHoldingsPrice = fill.FillPrice;
                        quantityHoldings = Math.Abs(fill.FillQuantity) * factor;
                    }
                    else
                    {
                        Log.Error($"quantityHoldings == 0 && order.Offset == OrderOffset.Close, OrderId: {fill.OrderId}");
                        return;
                    }
                }
                else if (isLong)
            ...
            //Set the results back to the vehicle.
            holding.AveragePrice = averageHoldingsPrice;
        }
    }
}
