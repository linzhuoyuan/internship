...
namespace QuantConnect.Securities
{
    ...
    public class SecurityService : ISecurityService
    {
            ...
            bool addToSymbolCache = true)
        {
            ...
            {
                defaultQuoteCurrency = symbol.Value[3..];
            }

            if (symbol.ID.SecurityType == SecurityType.Crypto && symbol.SymbolProperties == null && !_symbolPropertiesDatabase.ContainsKey(symbol.ID.Market, symbol, symbol.ID.SecurityType))
            {
                throw new ArgumentException($"Symbol can't be found in the Symbol Properties Database: {symbol.Value}");
            }
            
            var symbolProperties = symbol.SymbolProperties ?? _symbolPropertiesDatabase.GetSymbolProperties(symbol.ID.Market, symbol, symbol.ID.SecurityType, defaultQuoteCurrency);
            //反向赋值
            ...
            if (symbol.ID.SecurityType is SecurityType.Forex or SecurityType.Crypto)
            {
                ...
                {
                    // since we have none it's safe to say the conversion is zero\
                    if (CashBook.CryptoUsdToken.Contains(quoteCurrency))
                    {
                        _cashBook.Add(baseCurrency, 0, 0, quoteCurrency);
                    }
                    else
                    {
