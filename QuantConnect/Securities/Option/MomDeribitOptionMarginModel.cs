...
namespace QuantConnect.Securities.Option
{
    ...
    /// </summary>
    public class MomDeribitOptionMarginModel : SecurityMarginModel
    {
        ...
        protected override decimal GetMarginRemaining(SecurityPortfolioManager portfolio, Security security, OrderDirection direction, 
            OrderOffset offset = OrderOffset.None, long orderId = long.MaxValue)
        {
        ...
        protected override decimal GetMaintenanceMargin(Security security)
        {
           ...
           : Math.Max(0, underlying.Close - option.StrikePrice);
            ...
            if (optionRight == OptionRight.Call)
            {
                ...
                return (option.Close + Math.Max(OptionMarginParameter1 * underlying.Close - amountOTM, OptionMarginParameter2 * underlying.PreDayClose)) * option.SymbolProperties.ContractMultiplier;
            }
            //认沽期权义务仓维持保证金＝Min[合约结算价 + Max（12 %×合标的收盘价 - 认沽期权虚值，7 %×行权价格），行权价格]×合约单位
            return Math.Min(option.Close + Math.Max(OptionMarginParameter1 * underlying.Close - amountOTM, OptionMarginParameter2 * option.StrikePrice), option.StrikePrice) * option.SymbolProperties.ContractMultiplier;
        ...
        private decimal GetInitialMarginRequirement(Security security)
        {
            var option = (Option)security;
            var underlying = option.Underlying;
            if (option.Close == 0m ||
