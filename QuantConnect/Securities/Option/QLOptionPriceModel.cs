    ...
    class QLOptionPriceModel : IOptionPriceModel
    {
        ...
        public OptionPriceModelResult Evaluate(Security security, Slice slice, OptionContract contract)
        {
            try
            {
                ...
                // convert underlying price for crypto option, underlying is always 1 crypto.
                if (optionSecurity.Underlying.Type == SecurityType.Crypto)
                {
                    underlyingQuoteValue = new SimpleQuote(1);
                }
                ...
                if (optionSecurity.Underlying.Type == SecurityType.Crypto)
                {
                    payoff = new PlainVanillaPayoff(contract.Right == OptionRight.Call ? QLNet.Option.Type.Call : QLNet.Option.Type.Put, (double)contract.Strike / (double)optionSecurity.Underlying.Price);
                }
                ...
                double impliedVolatility = decimal.ToDouble(contract.PricingVolatility); 
                try
                {
                    impliedVolatility = option.impliedVolatility((double)optionSecurity.Price, stochasticProcess);
                }
                catch (Exception err)
                {
                    Log.Debug("QLNet implied volatility error: " + err.Message);
                }
                
                var pricingVolValue = new SimpleQuote(impliedVolatility);
                // 定价引用定价波动率计算
                if (contract.PricingVolatility != 0)
                {
                    pricingVolValue = new SimpleQuote(decimal.ToDouble(contract.PricingVolatility));
                }
