...
namespace QuantConnect.Securities
{
    ...
    public class OptionFilterUniverse : IDerivativeSecurityFilterUniverse
    {
        ...
        {
            // memorization map for ApplyOptionTypesFilter()
            var memorizedMap = new Dictionary<DateTime, bool>();

            bool MemorizedIsStandardType(Symbol symbol)
            {
                var dt = symbol.ID.Date;

                if (memorizedMap.ContainsKey(dt)) return memorizedMap[dt];
            ...
            var filtered = _allSymbols.Where(x =>
            {
                ...
                {
                    case Type.Weeklys:
                        return !MemorizedIsStandardType(x);
                    case Type.Standard:
                        return MemorizedIsStandardType(x);
        ...
        public OptionFilterUniverse Strikes(int minStrike, int maxStrike)
        {
            ...
            if (index < 0)
            {
                ...
                // 判断是否有大于当前标的价格的行权价的合约，如果没有就返回空的合约链
