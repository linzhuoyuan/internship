...
namespace QuantConnect.Securities
{
    ...
    public class SecurityExchangeHours
    {
        ...
        internal void SetHolidays(IEnumerable<DateTime> holidays)
        {
            _holidays.Clear();
            foreach (var holiday in holidays)
            {
                _holidays.Add(holiday.Ticks);
            }
        }

        internal void SetEarlyCloses(IDictionary<DateTime, TimeSpan> earlyCloses)
        {
            _earlyCloses.Clear();
            foreach (var (key, value) in earlyCloses)
            {
                _earlyCloses.Add(key, value);
            }
        ...
        public static SecurityExchangeHours AlwaysOpen(DateTimeZone timeZone)
        {
            ...
            return new SecurityExchangeHours(
                ...
                dayOfWeeks.Select(LocalMarketHours.OpenAllDay)
        ...
        /// in Closed instants when not present
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        {
            return _earlyCloses.TryGetValue(localDateTime.Date, out var earlyCloseTime) && localDateTime.TimeOfDay >= earlyCloseTime;
        }
