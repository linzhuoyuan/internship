...
namespace QuantConnect
{
    ...
    public class Isolator
    {
        ...
        /// <param name="codeBlock">Action code-block to execute</param>
            ...
            int sleepIntervalMillis = 1000)
        {
            ...
            while (!task.IsCompleted && DateTime.Now < end)
            {
                ...
                if (DateTime.Now > memoryLogger)
                {
                    ...
                    Log.Trace("Isolator.ExecuteWithTimeLimit(): " +
                              $"Used: {PrettyFormatRam(memoryUsed)} MB, " +
                              $"Sample: {PrettyFormatRam((long)sample)} MB, " +
                              $"App: {PrettyFormatRam(OS.ApplicationMemoryUsed * 1024 * 1024)} MB, " +
            ...
            if (message != "")
            {
                ...
                Log.Error($"Security.ExecuteWithTimeLimit(): The current time step consumed a period of {isolatorLimitResult.CurrentTimeStepElapsed:mm':'ss'.'fff}");
