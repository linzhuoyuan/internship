...
namespace QuantConnect.Statistics
{
    ...
    public class TradeBuilder : ITradeBuilder
    {
        ...
        private readonly ConcurrentDictionary<Symbol, Position> _positions = new ConcurrentDictionary<Symbol, Position>();
        ...
        private void ProcessFillUsingFillToFill(OrderEvent fill, decimal orderFee, decimal conversionRate, decimal multiplier)
        {
            ...
            if (!_positions.TryGetValue(fill.Symbol, out position) || position.PendingTrades.Count == 0)
            {
                ...
                Position addingPosition = new Position
                {
                    PendingTrades = new List<Trade>
                    {
                        new Trade
                ...
                _positions.TryAdd(fill.Symbol, addingPosition);
                return;
            }
            ...
            else
            {
                ...
                {
                    Position removedPos;
                    _positions.TryRemove(fill.Symbol, out removedPos);
                }
