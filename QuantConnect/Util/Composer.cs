...
namespace QuantConnect.Util
{
    ...
    public class Composer
    {
        ...
        private static readonly Lazy<Composer> LazyComposer = new Lazy<Composer>(
            () =>
            {
                _pluginDirectory = Config.Get("plugin-directory");
                return new Composer();
            });
        ...
        public Composer()
        {
            ...
            var loadFromPluginDir = !string.IsNullOrWhiteSpace(_pluginDirectory)
                && Directory.Exists(_pluginDirectory) &&
                new DirectoryInfo(_pluginDirectory).FullName != primaryDllLookupDirectory;
            _composableParts = Task.Run(() =>
            {
                    ...
                    {
                        catalogs.Add(new DirectoryCatalog(_pluginDirectory, "*.dll"));
                    }
            ...
            if (loadFromPluginDir)
            {
                fileNames = fileNames.Concat(Directory.EnumerateFiles(_pluginDirectory, $"{nameof(QuantConnect)}.*.dll"));
            }
        ...
        public void AddPart<T>(T instance)
        {
            ...
            {
                if (_exportedValues.TryGetValue(typeof (T), out var values))
                {
            ...
            where T : class
        {
            try
            {
                lock (_exportedValuesLockObject)
                {
                    T instance = null;
                    var type = typeof(T);
                    if (_exportedValues.TryGetValue(type, out var values))
                    {
                    ...
                    var typeT = _exportedTypes.FirstOrDefault(t => type.IsAssignableFrom(t) && t.MatchesTypeName(typeName));
                    if (typeT != null)
                    {
