...
namespace QuantConnect.Brokerages
{
    ...
    public class DefaultBrokerageModel : IBrokerageModel
    {
        ...
        public virtual IFillModel GetFillModel(Security security)
        {
            ...
            var nameSpace = "QuantConnect.Orders.Fills";
            var className = Config.Get("default-fill-model", "ImmediateFillModel");
            if (Config.Get("environment", "").Split('-')[0] == "backtesting" && security.Type == SecurityType.Option &&
                security.Symbol.ID.Market == Market.Deribit)
            {
                className = "DeribitOptionFillModel";
            }
            string fullName = nameSpace + "." + className;//命名空间.类型名
            Type type = Type.GetType(fullName);
            dynamic obj = Activator.CreateInstance(type,true);
            return (IFillModel)obj;
        }
        ...
        public virtual IFeeModel GetFeeModel(Security security)
        {
            switch (security.Type)
            {
                ...
                case SecurityType.Crypto:
                    ...
                    return new ProportionFeeModel(0.001m);

                case SecurityType.Equity:
                case SecurityType.Option:
                case SecurityType.Future:
                    if (security.Symbol.IsChinaMarket())
                        return new SseBrokersFeeModel();
                    return security.Symbol.ID.Market switch
                    {
                        Market.Okex => new OkexFeeModel(),
                        Market.Deribit => new DeribitFeeModel(),
                        Market.Binance => new ProportionFeeModel(0.001m),
                        _ => new InteractiveBrokersFeeModel()
                    };
                case SecurityType.Commodity:
                default:
                    return new ConstantFeeModel(0m);
        ...
        {
            if (Config.GetBool("infinite-buying-power"))
            {
            ...
            {
                case SecurityType.Equity:
                    switch (security.Symbol.ID.Market)
                    {
                        case Market.SSE:
                            model = new SseEquityMarginModel();
                            break;
                        default:
                ...
                case SecurityType.Option:
                    ...
                    {
                        case Market.SSE:
                            model = new SseOptionMarginModel();
                            break;
                        case Market.Deribit:
                            model = new DeribitOptionMarginModel(RequiredFreeBuyingPowerPercent);
                            break;
                        default:
                ...
                case SecurityType.Future:
                    ...
                    {
                        case Market.Deribit:
                            model = new DeribitFutureMarginModel(RequiredFreeBuyingPowerPercent);
                            break;
                        default:
