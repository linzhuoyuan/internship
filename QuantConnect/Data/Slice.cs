...
namespace QuantConnect.Data
{
    ...
    public class Slice : IEnumerable<KeyValuePair<Symbol, BaseData>>
    {
        ...
        private readonly SymbolBonusEvents _symbolBonusEvents;
        ...
        /// </summary>
        public TradeBars Bars => _bars;

        /// <summary>
        /// Gets the <see cref="QuoteBars"/> for this slice of data
        /// </summary>
        public QuoteBars QuoteBars => _quoteBars;

        /// <summary>
        /// Gets the <see cref="Ticks"/> for this slice of data
        /// </summary>
        public Ticks Ticks => _ticks;

        /// <summary>
        /// Gets the <see cref="OptionChains"/> for this slice of data
        /// </summary>
        public OptionChains OptionChains => _optionChains;

        /// <summary>
        /// Gets the <see cref="FuturesChains"/> for this slice of data
        /// </summary>
        public FuturesChains FuturesChains => _futuresChains;

        /// <summary>
        /// Gets the <see cref="FuturesChains"/> for this slice of data
        /// </summary>
        public FuturesChains FutureChains => _futuresChains;

        /// <summary>
        /// Gets the <see cref="Splits"/> for this slice of data
        /// </summary>
        public Splits Splits => _splits;

        /// <summary>
        /// Gets the <see cref="Dividends"/> for this slice of data
        /// </summary>
        public Dividends Dividends => _dividends;

        /// <summary>
        /// Gets the <see cref="Delistings"/> for this slice of data
        /// </summary>
        public Delistings Delistings => _delistings;

        /// <summary>
        /// Gets the <see cref="QuantConnect.Data.Market.SymbolChangedEvents"/> for this slice of data
        /// </summary>
        public SymbolChangedEvents SymbolChangedEvents => _symbolChangedEvents;

        public SymbolBonusEvents SymbolBonusEvents => _symbolBonusEvents;
        ...
        /// </summary>
        public IReadOnlyList<Symbol> Keys => new List<Symbol>(_data.Value.Keys);
        ...
        public Slice(
            ...
            Delistings delistings,
            SymbolChangedEvents symbolChanges,
            SymbolBonusEvents symbolBonusEvents,
            bool? hasData = null)
        {
            Time = time;
            ...
            _delistings = CreateCollection<Delistings, Delisting>(delistings);
            _symbolChangedEvents = CreateCollection<SymbolChangedEvents, SymbolChangedEvent>(symbolChanges);
            _symbolBonusEvents = CreateCollection<SymbolBonusEvents, SymbolBonusEvent>(symbolBonusEvents);
        }
        ...
        {
            get {
                if (_data.Value.TryGetValue(symbol, out var value))
                {
                    return value.GetData();
                }
                throw new KeyNotFoundException(string.Format("'{0}' wasn't found in the Slice object, likely because there was no-data at this moment in time and it wasn't possible to fill-forward historical data. Please check the data exists before accessing it with data.ContainsKey(\"{0}\")", symbol));
            }
        }
        ...
        public DataDictionary<T> Get<T>()
            where T : IBaseData
        {
            if (!_dataByType.TryGetValue(typeof(T), out var dictionary))
            {
        ...
        /// <param name="data">The data for the specified symbol, or null if no data was found</param>
        /// <returns>True if data was found, false otherwise</returns>
        public bool TryGetValue(Symbol symbol, out dynamic data)
        {
            data = null;
            if (_data.Value.TryGetValue(symbol, out var symbolData))
            {
        ...
        public void Clear()
        {
            ...
            _quoteBars.Clear();
            _optionChains.Clear();
            _futuresChains.Clear();
            _splits.Clear();
            _dividends.Clear();
            _symbolChangedEvents.Clear();
            _symbolBonusEvents.Clear();
        }
        ...
        private static DataDictionary<SymbolData> CreateDynamicDataDictionary(IEnumerable<BaseData> data)
        {
            ...
            {
                if (!allData.TryGetValue(datum.symbol, out var symbolData))
                {
                ...
                switch (datum.DataType)
                {
                    ...
                    case MarketDataType.Auxiliary:
                        symbolData.AuxiliaryData.Add(datum);
                        break;
        ...
        private Ticks CreateTicksCollection(Ticks ticks)
        {
            ...
            foreach (var listTicks in _data.Value.Values
                .Select(x => x.GetData())
                .OfType<List<Tick>>()
                .Where(x => x.Count != 0))
            {
        ...
        /// <summary>
        /// Returns the input collection if nona-null, otherwise produces one from the dynamic data dictionary
        /// </summary>
...
#pragma warning restore 618
        ...
        {
            // this will not enumerate auxiliary data!

            foreach (var pair in _data.Value)
            {
                ...
                if (data is IEnumerable<BaseData> dataPoints)
                {
                    ...
                    {
                        yield return new KeyValuePair<Symbol, BaseData>(pair.Key, dataPoint);
                    }
                }
                else if (data != null)
                {
                    yield return new KeyValuePair<Symbol, BaseData>(pair.Key, data);
                }
            }
        }
        ...
        private class SymbolData
        {
            ...
            public readonly List<BaseData> AuxiliaryData;

            public SymbolData(Symbol symbol)
            {
                Symbol = symbol;
                Ticks = new List<Tick>();
                AuxiliaryData = new List<BaseData>();
            }
