...
namespace QuantConnect.Data.Market
{
    ...
    /// </summary>
    public sealed class OptionChain : BaseData, IEnumerable<OptionContract>
    {
        ...
        internal OptionContracts contracts;
            ...
            IEnumerable<Symbol> filteredContracts)
        {
            ...
            this.ticks = new Ticks(time);
            this.tradeBars = new TradeBars(time);
            this.quoteBars = new QuoteBars(time);
            this.contracts = new OptionContracts(time);

            foreach (var trade in trades)
            {
                if (trade is Tick tick)
                {
                    if (!this.ticks.TryGetValue(tick.symbol, out var list))
                    {
                        list = new List<Tick>();
        ...
        {
            if (!_auxiliaryData.TryGetValue(typeof(T), out var dict) 
                || !dict.TryGetValue(target, out var list))
            {
                return default;
            }
        ...
        public List<T> GetAuxList<T>(Symbol target)
        {
            if (!_auxiliaryData.TryGetValue(typeof(T), out var dict) 
                || !dict.TryGetValue(target, out var list))
            {
        ...
        internal void AddAuxData(BaseData baseData)
        {
            ...
            if (!_auxiliaryData.TryGetValue(type, out var dict))
            {
                dict = new Dictionary<Symbol, List<BaseData>>();
                _auxiliaryData[type] = dict;
            }

            if (!dict.TryGetValue(baseData.symbol, out var list))
            {
                list = new List<BaseData>();
                dict[baseData.symbol] = list;
            }
            list.Add(baseData);
        }
    }
