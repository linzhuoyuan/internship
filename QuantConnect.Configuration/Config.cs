...
namespace QuantConnect.Configuration
{
    ...
    public static class Config
    {
        ...
        static Config()
        {
            ConfigurationFileName = PathHelper.CompletionPath("config.json");
        }
        ...
        /// <summary>
        /// Sets a configuration value. This is really only used to help testing. The key have can be
        ...
        public static void Set(string key, string value)
        {
            ...
            while (key.Contains("."))
            {
                var envName = key[..key.IndexOf(".", StringComparison.Ordinal)];
                key = key[(key.IndexOf(".", StringComparison.Ordinal) + 1)..];
                var environments = environment!["environments"];
                if (environments == null)
                {
                    environments = new JObject();
        ...
        public static JObject Flatten(JObject config, string overrideEnvironment)
        {
            ...
            if (!string.IsNullOrEmpty(overrideEnvironment))
            {
                ...
                for (int i = 0; i < environmentSections.Length; i++)
                {
                    ...
                    var settings = (environments as JObject)?.SelectToken(env);
                    ...
                    foreach (var token in settings)
                    {
                        ...
                        var value = (token is JProperty property ? property.Value : token).ToString();
